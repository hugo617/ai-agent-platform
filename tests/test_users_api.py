"""Users (tenant members) API tests — CRUD over the membership table."""
...
async def test_list_members_includes_owner(app_client):
    ...
    resp = await app_client.get(
        "/api/v1/users/", headers={"Authorization": "Bearer fake"}
    )
...
async def test_delete_member(app_client):
    ...
    resp = await app_client.delete(
        "/api/v1/users/dan", headers={"Authorization": "Bearer fake"}
    )
...
async def test_cannot_remove_self(app_client, tenant_owner):
    resp = await app_client.delete(
        f"/api/v1/users/{tenant_owner['user_id']}",
        headers={"Authorization": "Bearer fake"},
    )
