...
Issue/list/revoke require ``api_tokens:create`` / ``api_tokens:read`` /
``api_tokens:delete`` respectively (seeded for owner/admin). The coarse
``api_tokens:manage`` was split in the permission-unified-model task so a role
can, e.g., issue tokens but not revoke them. The ``/verify`` endpoint only
needs an authenticated caller: an external agent presents its token as a Bearer
credential (resolved by the ``ahp_`` bypass in ``get_current_user``) and gets
...
@router.post(
    ...
    dependencies=[Depends(require_permission("api_tokens", "create"))],
)
...
@router.get(
    ...
    dependencies=[Depends(require_permission("api_tokens", "read"))],
)
...
@router.delete(
    ...
    dependencies=[Depends(require_permission("api_tokens", "delete"))],
)
