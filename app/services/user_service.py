"""Member (tenant user) service — CRUD over the user_tenants association.

Each operation:
  1. checks the caller has the matching ``users:*`` permission in casbin, and
  2. keeps casbin's grouping policy (``g``) in sync with the DB role, so the
     change takes effect immediately for subsequent enforce() calls.
"""
...
class UserService:
    ...
    async def add(
        ...
        # Ensure the user row exists; create it on the fly if not (id = JWT sub).
        ...
        if existing is not None:
            ...
            # Re-load with the user relationship populated.
    ...
    async def remove(
        self, actor_id: str, tenant_id: str, target_user_id: str
    ) -> None:
