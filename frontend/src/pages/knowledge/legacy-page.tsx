/**
 * knowledge/ legacy-page — 旧 knowledge-page.tsx 整体迁移(reader-ui slice 01)。
 *
 * 旧 ``knowledge-page.tsx``(458 行单文件:列表 Table + 录入 Dialog + 删除 Dialog
 * + RetrievalDebugCard)在 reader-ui 切片 01 三栏化重构期间整体保留在这里,逻辑
 * **零变化**(plan G2 + AC5:现有行为零回归)。
 *
 * 切片 01 的 ``index.tsx`` 在三栏空壳下方渲染 ``<LegacyKnowledgePage/>``,所以
 * 录入/删除/检索调试的全部旧行为继续可用 —— 用户在切片 01 落地后仍能看到熟悉的
 * 旧列表 + CRUD + 调试卡,只是顶部多了三栏空壳(中栏新的卡片列表)。
 *
 * 切片 03 会把这里的 CRUD Dialog 迁进 ``document-list.tsx``、RetrievalDebugCard
 * 独立成 ``retrieval-debug-card.tsx``,然后**删除本文件**(plan §6 切片 03 AC)。
 * 本文件是过渡产物,不要在新代码里 import 它的符号(除 index.tsx 的过渡渲染外)。
 */
...
// statusBadge 从 shared.tsx 复用(reader-ui slice 01 抽出,避免重复声明)。
// 切片 03 删除本文件时,shared.tsx 的 statusBadge 继续供 document-list 使用。
