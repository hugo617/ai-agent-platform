...
// Since chat-stream-wallet-gate, /chat/stream enforces the same wallet gate
// pre-connection (402 before the stream starts, same detail body), so "402 =
// insufficient balance" is a shared contract across both chat paths — each
// keeps its own error class (the stream raises ChatInsufficientBalanceError)
// so its caller renders its own recharge prompt. Everything else falls
// through to `apiErrorMessage`.
