...
> 这是 **MVP 补全系列的总纲文档**(登记性质,✅ 已收口)。
> 2026-07-12 全面扫描识别出 12 个能力缺口,2026-07-14~16 全部落地(`feature_list.json` 全 passing)。
> **本系列已收官**,正文保留作历史规划记录;现状以 `feature_list.json` + 各 `plan-<id>.md` 顶部状态行为准。
...
### 扫描发现的 12 个能力缺口(2026-07-12 扫描登记,✅ 全部已交付)
...
> 状态: passing(PR #51 commit `0b0d397`,Session 083,2026-07-14)— 真实统计端点 + 趋势 + 门店/总部双视角
...
> 状态: passing(PR #52 commit `6f2f23b`,Session 085,2026-07-14)— SystemLog 查询 API + 审计日志页(双视角)
...
> 状态: passing(PR #54 commit `48f74b9`,Session 087,2026-07-14)— PUT /auth/me 改密码/资料 + 个人中心页
...
> 状态: passing(PR #55 commit `b6b8f3c`,Session 089,2026-07-14)— 对话搜索/重命名/标签/收藏/置顶/批量删除
...
> 状态: passing(PR #56 commit `70e7fba`,Session 091,2026-07-14)— 跨 Agent/客户/对话/用户聚合搜索 + 顶栏下拉
...
> 状态: passing(PR #57 commit `9073831`,Session 093,2026-07-14)— 租户 logo/名称/主题色/登录文案 + 主题色全局应用
...
> 状态: passing(PR #58 commit `f271f75`,Session 095,2026-07-14)— /ready 就绪探针 + /metrics Prometheus 指标
...
> 状态: passing(PR #59 commit `7fe629d`,Session 097,2026-07-14)— in-app 通知 + 铃铛 + APScheduler 定时任务
...
> 状态: passing(PR #60 commit `10b4ef0`,Session 099,2026-07-14)— CSV 流式导出(客户/对话/用量/审计,门店+总部)
- **现状**:无任何导出能力。
...
> 状态: passing(PR #62 commit `1643f2c`,Session 101,2026-07-14)— StorageBackend 抽象(Local/S3)+ /upload + FileUpload 组件
...
> 状态: passing(PR #66 commit `ea7c1ae`,Session 105,2026-07-16)— pgvector + Document/Chunk + embedding 管线 + retrieve_knowledge 工具
...
> 状态: passing(PR #67 commit `c75593f`,Session 106,2026-07-16)— Supervisor 编排 + agent_specialists M2M + 真实 DeepSeek 端到端
...
> 与已规划系列的关系:本系列 12 个交付项 + 权限重构(39-42)+ Token 计费(43-46)+ demo-seed-full(38) = 全部 passing(2026-07-14~16 收官)。WIP=1 顺序执行完毕,优先级由用户按业务需要排定。
...
- ✅ **本系列已全部落地**(2026-07-14~16),各 feature 的 plan 字段已指向独立 `plan-<id>.md`,evidence 见 `feature_list.json`
