...
> **状态**: draft v1(2026-07-30,第 8 次巡检候选 ④ Top)
...
### 切片 01:建 customers/ 文件夹 + 迁移组件 + tenantId/role smoke(非末切片)

**expand 阶段,不 git mv**(镜像 chat-page-split 切片01/02 范式,跨目录 import 成立):

- [ ] 1.1 新建 `customers/index.tsx`(route: `isSuperAdmin(me) ? <HqView/> : <StoreView/>`,从旧 CustomersPage L120-128 搬)
- [ ] 1.2 新建 `customers/shared.tsx`(`statusBadge` + `formSchema` + `FormValues`/`EMPTY_FORM` + `GENDERS`/`GENDER_LABEL``STATUSES` + `parseTagsJson` 纯函数)
- [ ] 1.3 新建 `customers/customer-usage-dialog.tsx`(`CustomerUsageDialog` + `Metric` 从旧文件搬,内部 import Metric)
- [ ] 1.4 新建 `customers/store-view.tsx`(从旧 StoreView L134-538 搬,改调 `parseTagsJson`,import shared 的 schema/常量/statusBadge,import customer-usage-dialog)
- [ ] 1.5 新建 `customers/hq-view.tsx`(从旧 HqView L659-830 搬,import shared 的 GENDER_LABEL/statusBadge,import customer-usage-dialog)
- [ ] 1.6 **`git mv pages/customers-page.tsx → pages/customers-page.tsx`(原地保留改内容)**:customers-page.tsx 改成 barrel(`export { CustomersPage } from "./customers/index"`,镜像 bookings-page.tsx)
- [ ] 1.7 App.tsx 不改(barrel 保路径);grep 确认无其他 `pages/customers-page` 残留外部引用
- [ ] 1.8 新建 `customers/__tests__/hq-view.test.tsx` smoke(2-3 tests):route 渲染 super_admin → HqView / 非 super_admin → StoreView(镜像 devices 切片1 v2 smoke 前移,消除空窗)
- [ ] 1.9 **验证**:`npm run build` 0 错 + `npm test` 全绿(基线 + smoke)+ `oxlint` 0/0 + grep `pages/customers-page` 外部 import 残留 = 0 + customers/ 文件夹 6 文件

**切片 01 完成标志**:customers/ 文件夹 6 文件就位 + 旧 customers-page.tsx 变 barrel + smoke 锁住 route 分支 + build/test 全绿。

### 切片 02:补完整 store-view + hq-view 单测(末切片)

- [ ] 2.1 新建 `customers/__tests__/store-view.test.tsx`(镜像 devices/store-view.test.tsx 范式,~5-6 tests):
  - [ ] 2.1.1 列表渲染(profile name / identity_key / status 徽章 / gender 标签)
  - [ ] 2.1.2 空态(无 profile → EmptyState)
  - [ ] 2.1.3 member 只读守卫(canCreate=false → 无「新增」按钮 + 无操作列)
  - [ ] 2.1.4 owner 创建 Dialog 填表提交触发 useCreateCustomerProfile(**断言 payload tags 经 parseTagsJson 正确解析**)
  - [ ] 2.1.5 编辑菜单触发(openEdit 填充 form + 提交触发 updateMut)
  - [ ] 2.1.6 删除菜单触发(deleteMut.mutateAsync 被调)
- [ ] 2.2 扩展 `hq-view.test.tsx`(切片1 smoke → 完整 ~5-6 tests):
  - [ ] 2.2.1 跨店表渲染(customer name / identity_key / profile_count 徽章 / gender)
  - [ ] 2.2.2 空态
  - [ ] 2.2.3 行展开(toggle → 显示 profiles 明细子行,含 tenant.name + statusBadge)
  - [ ] 2.2.4 搜索过滤(?search= 过滤 name/identity_key)
  - [ ] 2.2.5 AI 用量按钮触发(setUsageTarget → CustomerUsageDialog storeScoped={false})
- [ ] 2.3 新建 `customers/__tests__/parse-tags.test.ts`(D4 纯函数,3 边界用例):
  - [ ] 2.3.1 合法 JSON `{"level":"vip"}` → {tags: {level:"vip"}}
  - [ ] 2.3.2 非法 JSON `{broken` → {tags: undefined, error: "标签 JSON 格式错误"}
  - [ ] 2.3.3 空字符串/纯空白 → {tags: undefined}(无 error)
- [ ] 2.4 **验证**(plan §10 AC 全绿):`npm test` 全绿(基线 + 5 store + 5 hq + 3 parse-tags)+ `npm run build` 0 错 + `oxlint` 0/0 + grep 残留旧路径 = 0 + `./init.sh full` 后端零回归(纯前端)
- [ ] 2.5 **feature 收尾仪式**(three-tier §4 第1-8步):见 §6
...
- [ ] ① `./init.sh full` 全绿 + 前端 npm test + build + oxlint 全绿
- [ ] ② `feature_list.json` status `not_started → passing` + evidence 4 条(切片1/2 + parseTagsJson + 收尾条)
- [ ] ③ `./scripts/sync-active-features.sh` 刷新 active 视图
- [ ] ④ `progress.md` 顶部 frontier 清空 + 本条记录
- [ ] ⑤ `clean-state-checklist` 逐项 ✅
- [ ] ⑥ 文档影响评估:纯前端结构重构,**无新增/改动文档**(AGENTS.md/项目指南/铁律均不受影响)
- [ ] ⑦ **末切片依赖解锁扫描**:无任何 feature `depends_on` 指向 customers-page-split(纯重构无下游)→ 无需推进
