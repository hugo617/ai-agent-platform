...
- **当前最高优先级未完成功能**: **union-cast-split**(priority 75,not_started)。第 6 次巡检(2026-07-29 Session 163)Top recommendation —— 前端 5 个 role-branching hook 返回 union(`Device[]|DeviceHqRead[]` 等),窄化散落 4 view 靠 12 处 `as` 断言(微恶化,第5次 ~10→12)。grill 8 决策定方向:**拆 role-specific hook**(useDevices/useDevicesAll,union 在 hook 层消灭而非 view 边界)+ queryKey 共享 + All 后缀(随 useAllTenants 先例)+ 不提 ADR + ModelOption 投影 cast 不纳入(正交)。纯前端类型重构,零后端零 schema 零运行时行为变化,3 切片按 domain 分(bookings / devices / 收尾)。**plan**:`harness/docs/plan-union-cast.md` draft v1。下一步:EP3 `/implement` 切片 01(bookings domain,frontier)。twospace-config(priority 74)全 3 切片已于 Session 162 收官 passing;bookings-shared-split(priority 73)已于 Session 158 收官 passing。
...
- **当前 blocker**: 无
- **EP3 断点**: **union-cast-split 切片 02 待启(devices domain)**(切片 01 ✅ 已完成,2026-07-29 Session 164 非末切片)。**切片 01 ✅(bookings domain)**:消解 2 处 A 类 + 1 处 D 类 union-cast —— endpoints.ts 新增 `fetchBookingsAll`(返回 `BookingHqRead[]`,调同一 `/bookings/` URL)+ `fetchBookings` 返回类型从 union 收窄为 `Booking[]`(store 视角);queries.ts 新增 `useBookingsAll`(queryKey 共享 `qk.bookings`,D5)+ import 加 `fetchBookingsAll`;hq-view 改调 `useBookingsAll` 消 L143 `as BookingHqRead[]` + 清 Note(candidate-8) + 更新 docstring;store-view 消 L186 `as Booking[]`(useBookings 已返窄类型)+ 清 Note + 更新 docstring;my-bookings-view 删 L63 D 类死 cast(useMyBookings 走 /me/bookings 非 union)+ 删 Note + 纠正 docstring(原误称 union);hq-view.test.tsx mock 改名 `useBookings`→`useBookingsAll`(定义+工厂+9 调用点)。**未动**(跨切片中间态可编译可运行):hq-view L163 `as DeviceHqRead[]` + store-view L152/355/365 `as Device[]`(device cast 留切片 2)+ hq-view L518/521 B 类 props cast `as Booking`/`as BookingHqRead`(留原样,组件签名重构)+ C 类 `as ModelOption[]`/`as DeviceStatus` + `fetchBooking` 单数死代码 + 后端零改动 + store-view.test.tsx(mock 用 makeBooking 构造 Booking[],天然兼容)。**验证**:npm run build 0 类型错误(1.68s)+ npm test 65/65 全绿(零行为回归,mock 改名生效)+ npx oxlint 0/0 + grep `as Booking[]`/`as BookingHqRead[]` 在 pages/bookings/ **归 0**(唯一匹是 my-bookings docstring 描述性文字)+ Note(candidate-8) **0 处** + B 类 props cast 保留确认。**code-review 双轴(opus ×2 并行)**:Standards 0 硬违规 / 1 判断项(`fetchBookings`+`fetchBookingsAll` 函数体运行时相同仅泛型参差异 → Duplicated Code,被 D1 设计正当化,抽私有 fetcher 会重引入 union,可接受);Spec 8 AC 全满足 / 0 缺失 / 0 误 / 2 轻微 scope creep(① `fetchBookings` 返回类型收窄为 `Booking[]` —— 使 store-view cast 能消的必要前提 ② 注释扩写 ~30 行 plan 未禁止),均必要连带非越界。下一步:**切片 02 devices domain**(Blocked by 01 已解锁:新增 fetchDevicesAll/useDevicesAll + fetchDeviceModelsAll/useDeviceModelsAll,hq-view 消 L163 + store-view 消 L152/355/365 + devices-page 双组件消 L192/422 + device-models-page 消 L149/216,8 处 A 类 device cast)。
