...
- **当前最高优先级未完成功能**: **`device-booking`(priority 63,设备预约订单 CRUD + 排期,系列 3/4,status=`in_progress`,EP3 进行中)**。设备功能系列进度:61(device-models-crud)✅ → 62(devices-crud-ui)✅ 全 passing → **63(device-booking `in_progress`,plan-device-booking.md 含 7 切片 + 12 决策)** → 64(device-poweron)。切片级状态真相源 = `harness/docs/plan-device-booking.md` 的 acceptance criteria checklist。切片进度:**切片 01(后端地基)✅ 已合并 PR #106 commit f2bfc93** → **切片 02(权限 seed + backfill)✅ 已合并 PR #107** → **切片 03(HQ 全景 + 排期聚合后端)✅ 已合并 PR #108 commit 2a69176** → **切片 04(customer own 端点 GET /me/bookings)✅ 已合并 PR #110 commit 24685a4(代码全绿 651 passed + ruff 0 + /code-review 0 阻塞)** → **切片 05(前端地基 types/endpoints/queries + 路由 + nav)✅ 已合并 PR #111 commit 70a60eb(纯地基无 UI,代码全绿 651 passed + build + oxlint 0 + /code-review 0 阻塞)** → 切片 06(前端门店端 StoreView)待做(frontier)。**下一步**:device-booking 切片 06(前端门店端 StoreView:列表 Table + filter chips 今日/明日/本周/待确认/爽约 + 排期网格 slot-box booked/active/done 三态 + CRUD Dialog + 时段冲突 400 toast),走 `/implement`(EP3,从 frontier 切片接)。注:切片 06 blocked by 05,现 05 已合并,切片 06 可开。切片 05 已铺好全部 types/endpoints/queries 钩子(useBookings/useCreateBooking/useUpdateBooking/useCancelBooking/useDeviceSchedule/useMyBookings)+ /bookings 路由 + nav,切片 06 直接接 StoreView 实现。
- **当前 blocker**: 无
- **EP3 断点(Session 135)**:device-booking 切片 05 已合并(PR #111 commit 70a60eb)。切片 05 是后端→前端转折切片(纯地基无 UI),为切片 06/07 铺路:types.ts 加 Booking/BookingCreate/BookingUpdate/BookingStatus(6 态)/BookingHqRead/DeviceSchedule;endpoints.ts 加 7 个 booking API client(含 cancelBooking POST /cancel 非 DELETE);queries.ts 加 qk.bookings/deviceSchedule/myBookings + 7 hooks + BOOKING_WRITE_KEYS 失效集;App.tsx 加 /bookings 路由(裸 ProtectedRoute);nav-items.ts 业务管理 subgroup 加预约项(Calendar icon,menu:bookings);pages/bookings-page.tsx 最小占位。续做时直接开切片 06(前端门店端 StoreView),接切片 05 已铺好的全部钩子。
