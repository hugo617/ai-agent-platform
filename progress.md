...
- **当前最高优先级未完成功能**: **`device-models-crud`(priority 61,设备型号目录 CRUD 平台级主数据,Session 122 新登记)**。设备功能系列 4 条已登记 not_started(61-64,依赖链 61→62→63→64),未开工。最新合并到 main:`api-token-fine-grained-scopes`(priority 60)✅ ship-it 流水线零修复合入 —— API Token 细粒度 scope 收敛闭环(restricted/full 双模式 + contextvar 闸门,scope_mode 列 + 迁移 d6e7f8a9b0c1 + 前端 scope 矩阵 + 22 测试)。
- **当前 blocker**: 无
