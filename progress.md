...
- **当前最高优先级未完成功能**: **`knowledge-tiered-backend` p89(知识库分级 Feature B:后端 CRUD + 下发 API + 检索改造)— in_progress,EP3 推进中(切片 01+02+03 ✅,frontier=切片 04 集成验证+feature 收尾,末切片)**。plan=`harness/docs/plan-knowledge-tiered-backend.md`(EP2 产物,4 切片,3 已完成)。系列 4 feature:**p90 foundation ✅ passing 全 3 切片完成** / p89 backend in_progress(切片 01+02+03 完成,frontier=切片 04 末切片)/ p88 reader-ui not_started depends_on backend / p87 admin-ui not_started depends_on backend。**foundation 全 3 切片**:`01 数据模型地基 ✅ commit 73602e5`(2 改表+2 新表+1 内聚迁移 7 步+seed 5 Category,16 tests)→ `02 权限派生+自动化 ✅ commit 5e8c303`(is_group_admin 派生 helper + check() knowledge bypass[obj=='knowledge' and db → group_admin 放行,D9 仅知识库域]+ tenant_service.create_tenant 第7步单门店自成一集团 + AC8 事务回滚,15 tests)→ `03 集成验证+feature 收尾 ✅ commit ad633c5`(4 端到端集成测试:完整流程 HTTP 自动建集团→is_group_admin→check 放行 knowledge/拒 devices + 跨集团隔离 + 手工连锁分店owner非group_admin + distribution 引用语义;894 passed 零回归;双轴 review 共识采纳:复用 _seed_user_role helper + 删 AC4 越界 effective 断言避免钉死 Feature B list 逻辑 + AC1/AC3 docstring 补充)。**backend 切片进度(EP3)**:`01 Category CRUD+scope 分级 ✅ commit e7297cd`(KnowledgeCategoryService+Repo list_visible 三路径 + schemas + 4 端点,31 tests;双轴 review:Standards 1 硬违规 repo→service 反向 import→bool 下传修复 + 3 判断项采纳)+ `02 list+检索三路径+G1 bypass 接通 ✅ commit d21f8a9`(跨 scope 可见性核心兑现 + group_admin bypass 真正生效[list_visible_for/search_by_embedding 三路径 + KnowledgeService 4 require + graph check 全加 db + DocumentRead 加 scope/group_id/category_id + retrieve_knowledge 工具 include_distributed=True],16 tests,940 passed 零回归;+1 flaky composite timeout 计时器竞态无关)+ `03 下发/撤回 API + distribute 权限码 ✅`(DistributeRequest G4 XOR[service BizError 非 schema validator,避 422 坑]+ KnowledgeDistributionRepository 新建[pre-check upsert find_for_pair 非 IntegrityError catch —— SQLite/PG flush timing 分叉 + rollback 丢 pending]+ distribute_document[target_group_id 展开 GroupTenant.list_for_group,group_admin 跨集团 is_group_admin 校验拒]+ revoke_distribution[软删 is_active=false 保留审计]+ 2 端点 + DEFAULT_OWNER/ADMIN_PERMS 加 distribute[member 不加]+ ACT_CN「下发」+ BACKFILLABLE_OBJS 加 knowledge;**权限码走 runtime seed 路径非 migration**(与 devices/bookings 先例字节对齐,偏离 plan 原文 raw SQL 决策 —— raw SQL 只碰 1/3 表违 SCD2+casbin 同步铁律);35 tests,979 passed 零回归)。**核心架构**:三级权限(super_admin / group_admin 派生身份=总部门店 owner/admin / 门店 owner/admin/member)+ 双维度分类(scope + category)+ 显式下发(引用模型,撤回软删)+ 三栏阅读。「门店=最小 OPC 产业单元」。第 9 次巡检候选 ①+④ 已收官(p84+p85)。**远端 main 真实 HEAD = `e75d811`**(origin/main 实测;foundation 末切片 + §8 status 行修正)。最近 passing:knowledge-tiered-foundation(90)/member-service-direct-tests(85)/user-service-lookup-seam(84)/design-system-spacing-card-hierarchy(83,C)/design-system-color-sweep(82,B)。
...
- **EP3 断点**: **knowledge-tiered-backend 切片 01+02+03 ✅ 完成 —— frontier=切片 04 集成验证+feature 收尾(末切片)**(2026-08-07 Session 续)。切片 01(`e7297cd`)+ 切片 02(`d21f8a9`)+ 切片 03(本次)代码 + 测试 + 收尾文档全落,`./init.sh full` **979 passed** 零回归。切片 03 兑现:D3「显式下发」语义端到端跑通(DistributeRequest G4 XOR + KnowledgeDistributionRepository pre-check upsert + distribute_document/revoke_distribution + 2 端点)+ distribute 权限码(runtime seed 路径,与 devices/bookings 一致)+ 源文档软删联动联合谓词证明。**下一步:EP3 `/implement` 切片 04**(末切片:6 集成测试[完整下发链路/group_admin 链路/Category 跨级/源文档软删联动/跨租户隔离/D9 越界守卫]+ ./init.sh full + feature 收尾仪式[three-tier §4 第1-7步:status→passing + evidence + sync-active + progress.md + 文档影响评估 + 依赖解锁扫描 C/D depends_on=backend 满足])。blocked by 切片 03 已解锁。
