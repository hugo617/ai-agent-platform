...
- **当前最高优先级未完成功能**: **risk-hardening 系列(生产加固,EP1 已完成)—— frontier = `super-admin-write-audit`(priority 92,R4 三处最高危,需各自 EP2 回环 /grill→/to-spec→/to-tickets)**;系列剩余:92 super-admin-write-audit → 91 config-startup-guard(R5 三件套),WIP=1 按 priority 串行。**billing-reconciliation-job(93)✅ passing(2026-08-17 Session 221,全 2 切片端到端,feature 收官)**:切片 01 PR #171(merge 28a2034,CI 4/4 绿:Migrations 43s/Backend 11m14s/E2E 1m56s/Frontend 28s)——`BillingReconciliationService.run(as_of,force)` 双层检出(事件级 NOT EXISTS 漏扣明细[LOOKBACK_BUFFER=30min 单源]+ 残余差共用 cutoff 窗口 + 钱包不变式带符号求和)+ 每 run 一条 SystemLog(直写不走 LoggingService)+ as_of 日粒度幂等锁 + 事件首告去重 + scheduler 09:30 薄壳,11 用例 TDD;code-review Standards 0🔴/4🟡(1 修 3 留痕)/ Spec 8/8 + §4.7 实施注记。**切片 02 末切片(Session 221,commits 0964f92 + bd42518)**:超管 targeted 通知 ——`_super_admin_targets`(全体 super_admin AND is_deleted=False,首个 active membership[valid_to IS NULL,earliest valid_from+id tiebreak]租户,无 membership logger.warning 跳过)+ `_notify_super_admins`(type=system/title=计费对账发现差额/link=/logs[App.tsx 实测审计页路由];调用点 SystemLog commit 之后,**整个 notify stage** try/except best-effort,通知侧异常不外抛)+ CONTEXT.md 术语「对账(Reconciliation)」(双层口径/只报不补/首告去重,glossary 零实现细节)+ 用例 8 四条(8a targeting 字段逐项+双 membership 落点/8b 无超管不崩 run 记录仍落/8c 干净 run 超管在场也零通知/8d 通知 stage 抛错 run 不抛)TDD 红→绿;全量 **1063 passed 零回归**(基线 1059+4)+ build 绿(零前端改动确认)+ ruff;code-review Standards 0🔴/4🟡(2 修[best-effort 扩全 stage + 删死返回值]2 留痕[run() 拆分留巡检 + 裸 select 豁免延伸 User/UserTenant 入档 §4.7])/ Spec 0 缺失 0 越界(「首个」tiebreak 消歧入档);feature 收尾 8 步闭环(status passing + evidence 3 条 + plan 状态行同 commit 翻 passing + check_plan_status_sync exit 0 + sync-active + 依赖解锁扫描[无 depends_on 指向,新 frontier=92]+ 分支清理 PR 合并后)。**chat-stream-wallet-gate(94)✅ passing(2026-08-17 Session 218,全 2 切片端到端,feature 收官)**:切片 01 PR #169(merge 99d1fd2,`_require_wallet_balance` 共享 helper + SSE 预检 402 + 移除流内旧门 + funded_wallet + 矩阵 ①-⑧,全量 1048 零回归)+ **切片 02 末切片(Session 218)**:前端 402 充值引导(search.ts `ChatInsufficientBalanceError`[fetch 手动容错解析 body detail,非 JSON 兜底后端同文案]+ chat/index.tsx balanceError state + 充值面板镜像 composite AC4.8[Wallet icon/text-foreground WCAG/前往充值 → /billing]+ 双清除[发送前 + 切换会话]+ localMessages **快照回滚** `localBefore`[被拦不留假消息/永久 typing 占位,且不清掉同会话上一轮流式内容——Standards 轴判断项② 修复]+ composite-chat.ts 头部注释统一口径)+ 测试 8 条(单元 4 + 组件 4)TDD 先红后绿;前端 259/259(基线 251 + 8)+ build + oxlint 0/0 + `./init.sh full` 1048 passed 零回归;code-review Standards 0🔴/3🟡(1 修 2 留痕)/ Spec 0 缺失 0 越界(回滚判 D4 合理推导入档 plan AC2 注记);feature 收尾 8 步闭环(status passing + evidence 3 条 + plan 状态行同 commit 翻 + sync-active + 依赖解锁扫描[无 depends_on 指向,新 frontier = billing-reconciliation-job 93])+ 分支清理。**booking-toctou-guard(95)✅ passing(2026-08-16 Session 215,全 2 切片端到端,feature 收官)**:切片 01 PR #167(merge 596dccd,EXCLUDE 约束 + 预检拒迁 + PG 门控①-⑤ + CI Migrations step)+ **切片 02 末切片(Session 215)**:服务层 23P01→400 双路径映射(create repo.add 捕获 / update db.flush 捕获,`_map_exclusion_violation` 纯判别 helper,非 23P01 一律 re-raise)+ 判别单测 3(SQLite X 章节)+ PG 用例 ⑥ 竞态全链端到端(pg_stat_activity 确定性握手,断言 BizError 而非 IntegrityError)+ CONTEXT.md「占坑态(Slot-Holding States)」+ 数据库 ORM 文档 PG-only EXCLUDE 范式段;全量 **1043 passed 零回归** + PG 门控 8/8 + alembic 干净 + 前端 build 零改动;code-review Standards 0🔴/4🟡(2 修 2 留痕)/ Spec 6/6 无越界;feature 收尾 8 步闭环 + 分支清理。**切片 01 已完成 2026-08-15 Session 214**:commits 4ced028+50eaa67,PR #167(merge 596dccd,CI 4/4 绿含新 Migrations step「Postgres-gated booking-overlap tests」),迁移 9a8b7c6d5e4f(EXCLUDE 约束 excl_bookings_active_no_overlap + 预检拒迁两条 + btree_gist,常量单源插值 4 SQL 站点)+ PG 门控测试 ①-⑤ 7/7 + 同源防漂移测试 2 用例 + booking.py docstring + CI wiring;TDD 红证 + 拒迁实测(exit 1 带数量)+ downgrade pg_catalog 确认 + alembic check 干净 + ./init.sh full 1040 passed 零回归;code-review Standards 0🔴/3🟡(1 修 PG 测试状态清单单源)/ Spec 7/7 无越界;竞态双订已被 DB 拦下(裸 IntegrityError → 500,切片 02 把它收口 400 并收官)。EP2 已完成 2026-08-15 Session 213(8 项决策用户逐项拍板 + 双轴审查回写 v2 双切片就绪,详见 Session 213)。**rate-limit-login-lockout(96)✅ passing(2026-08-15 Session 212,全 3 切片端到端)**:切片 03 末切片 TTL 收口 + 前端 429 事件桥 + feature 收尾(commits aa8be67+c7c65a7,PR #166,merge da02317,CI 4/4 绿:TTL 480/8 + .env.example 统一消除 60 分歧 + client.ts 429 事件桥 + RateLimitedToast listener + CONTEXT.md 术语「临时锁定」;后端 1038 前端 251 全绿零回归;code-review Standards 0 硬违规[配置指南表失真已修]+ Spec 5/5 无越界;feature_list in_progress→passing + evidence 4 条 + plan 状态行同 commit 翻 passing,check_plan_status_sync exit 0)。**切片 02 slowapi 全局限流已完成 2026-08-14 Session 211**:commits 5198b4f+84b30db+0599c5f,PR #165,9 AC 全勾,全量 1036 零回归(基线 1020+新增 16),CI 4/4 绿(E2E 2m5s);Spec 轴抓获 CORS 栈序问题已修(默认档 429 补 ACAO);CI E2E 抓获 slowapi 装饰器+pydantic 返回值陷阱已修(login 补 response: Response 参数);**切片 01 登录失败锁定已完成 2026-08-14 Session 210**:commit b1c3268,PR #164(merge 888a70b,CI 4/4 绿),12 AC 全勾,全量 1020 零回归;feature_list 已翻 in_progress[commit 0aa8220 同步状态行]。**EP2 已完成 2026-08-14 Session 209**:plan v2 三切片就绪 [harness/docs/plan-rate-limit-login-lockout.md](harness/docs/plan-rate-limit-login-lockout.md),11 项决策全用户拍板)。EP1 已登记 6 条 not_started(pri 96-91,总纲 [harness/docs/plan-risk-hardening-overview.md](harness/docs/plan-risk-hardening-overview.md),8 决策全用户拍板无默认采纳):96 rate-limit-login-lockout / 95 booking-toctou-guard(R2 TOCTOU DB 兜底)/ 94 chat-stream-wallet-gate(R3 前半,止血)/ 93 billing-reconciliation-job(R3 后半,挂 APScheduler 对账)/ 92 super-admin-write-audit(R4 三处最高危)/ 91 config-startup-guard(R5 三件套)。六条相互独立,WIP=1 按 priority 串行,每条 EP2 时阈值类取舍(锁定几次/多久、TTL 值、配额分级等)必须问用户拍板(96 已完成此步)。系列收官 = 全 passing + 第 11 次巡检复验 Top5 清零。最近收官:perm-check-bypass(86,权限-后端)/knowledge-tiered 系列(87-90)。**网络说明**:github.com:443 间歇不可达,git push/fetch 需 `-c http.version=HTTP/1.1` 且可能重试多次;`gh`/`gh api`(api.github.com)全程稳定,远端验真以 gh api 为准。最近 passing:billing-reconciliation-job(93)/chat-stream-wallet-gate(94)/booking-toctou-guard(95)/rate-limit-login-lockout(96)/perm-check-bypass(86)。
...
### 实施(commit 0964f92,TDD 红→绿)

落地 1 改 service + 1 改测试 + 1 改 CONTEXT:① `app/services/billing_reconciliation_service.py` 通知编排 ——`_super_admin_targets()`(全体 `platform_role='super_admin' AND is_deleted=False`,JOIN-free 双查询:超管全体 + 各自 active memberships[valid_to IS NULL],`ORDER BY user_id, valid_from, id` 后 Python 侧 setdefault 取首个,无 membership 置 None 哨兵)+ `_notify_super_admins(message)`(每超管经首个 active membership 租户发 targeted 通知 `type="system"/title=计费对账发现差额/content=摘要含新告数存量数残余差/link="/logs"`[App.tsx 实测审计页路由],无 membership logger.warning 跳过,`NotificationService.create` SAVEPOINT 内 flush → 编排末尾一次 commit,镜像 scan_balance_warnings);调用点在 `has_discrepancy` 分支、SystemLog commit **之后**(run 记录是主产物,通知是 best-effort 副产物)。② `CONTEXT.md`「Token 计费」节末新增术语「对账(Reconciliation)」(双层口径/只报不补/首告去重三要点,glossary 格式零实现细节)。③ `tests/test_billing_reconciliation.py` 用例 8 三条:8a 超管收到通知(targeting 字段逐项断言:user_id/tenant_id=首个 membership 租户[双 membership 显式 valid_from 钉死排序防 SQLite 秒级 tie]/type/title/content/link + 无 membership 超管 caplog warning)/ 8b 无超管不崩(run 记录仍落)/ 8c 干净 run 零通知(超管在场也不发——锁「零差额→零通知」而非「没超管」)。红证:8a 于零通知行失败。

### /code-review 双轴(general-purpose ×2 并行)→ 回写(本 commit)

**Standards 0🔴/4🟡**:修 2 —— ① best-effort 边界:run() 调用点把**整个 notify stage**(target 查询+插入+commit)包 try/except logger.exception,通知侧任何异常不外抛(补用例 8d 锁:monkeypatch `_notify_super_admins` 抛 RuntimeError,run 不抛 + run 记录仍落);② `_notify_super_admins` 死返回值 created 删除(scan_balance_warnings 的计数是 job 返回契约,此处无人消费,Speculative Generality)。留痕 2 —— ③ run() ~168 行拆 `_build_report` 属纯机械移动,按不越界原则留独立 commit/巡检;④ 切片 01 的「平台级跨租户只读 job 裸 select 豁免」延伸至 User/UserTenant,plan §4.7 实施注记点名留痕防豁免无声扩大。**Spec 0 缺失 / 0 越界**:AC 逐项吻合(targeting 字面/触发条件字面/用例 8 三条/CONTEXT 三要点/验证 1062+build+ruff 全绿/diff 仅 3 文件);1 spec 空白消歧——「首个 active membership」tiebreak 定死 earliest valid_from(+id),用例 8a 双 membership 锁定;feature 收尾八步在后续收尾 commit 闭环(非 diff 缺陷)。

### 验证

定向 `pytest tests/test_billing_reconciliation.py -q` **15/15**(11 旧 + 4 新零回归);ruff 全绿;全量 `pytest -ra --strict-markers` 收尾复跑 **1063 passed, 8 skipped**(基线 1059 + 新增 4,零回归;8 skipped = PG 门控);前端 `npm run build` 绿(1.93s,零前端改动确认)。
...
**任务**:EP3 切片 01 — `BillingReconciliationService` 双层检出 + 幂等/首告去重 + SystemLog 落表 + logger 通道 + scheduler 09:30 薄壳。分支 `feat/billing-reconciliation-job-slice-01`(自 main 43b59de 新开),开工冒烟 `./init.sh` 绿。开工翻页 commit 130f20b(feature_list not_started→in_progress + plan 状态行同 commit 翻,check_plan_status_sync exit 0)。

### 实施(commit 0a0d8f9,TDD 4 批次红→绿,11 用例)

落地 1 新 service + 1 改 scheduler + 1 新测试:① `app/services/billing_reconciliation_service.py`(374 行)——`run(as_of, force)`:事件级 NOT EXISTS 漏扣明细(`LOOKBACK_BUFFER=30min` 常量单源)+ per-tenant 聚合残余差(事件侧/交易侧**共用 cutoff 窗口**,漏扣两侧对消只报交易侧异常:SET NULL 孤儿/手工改动/多扣)+ 钱包终身不变式(refund/adjust **带符号**求和,负向 adjust 不误报);每 run 恰一条 SystemLog(action=billing_reconciliation,details_json 全量报告,差额 warning/干净 info;**直写不走 LoggingService**——run 记录是主产物,吞错会静默破坏双幂等锁,docstring 书面论证);as_of 日粒度幂等锁(同日 skip 返回摘要不插记录,force 绕过)+ 事件首告去重(已告集合=全部历史 run 的 new_alerted_event_ids 并集,details_json 只存本 run 新告防膨胀;存量未处理单独计数);naive as_of 按 UTC 解释,全程 aware-UTC。② `app/core/scheduler.py`——`reconcile_billing` 薄壳(镜像 scan_balance_warnings 签名,as_of=now,异常 logger.exception 返 None 不崩)+ `_register_jobs` CronTrigger(9,30) 注册 + 模块头 docstring 同步。③ `tests/test_billing_reconciliation.py`(534 行 11 用例)——SQLite 常驻直调范式(factory 绑 StaticPool 同连接,fixed `AS_OF` + 显式 created_at 钉死缓冲边界 −5min/−31min;对照组走**真实 BillingService.charge 链路**后钉交易时间戳防真实时钟漂移;run 记录按 as_of 选取防 SQLite 秒级 CURRENT_TIMESTAMP 同秒排序退化)。用例映射:1 漏扣检出双租户不串/2 干净 info/3a 同日 skip+次日正常/3b force 不重告/4 首告+存量分账(day1 告 A day2 只告 B)/5 缓冲双边界/6 孤儿 consume 残余差(事件级零漏扣也能检出)/7 手改 balance 漂移 + 薄壳吞错 + 薄壳真跑 + 注册 09:30 共 11。

### /code-review 双轴(general-purpose ×2 并行)→ 回写(本 commit)

**Standards 0🔴/4🟡**:修 1 —— `ra_by_wallet` Mysterious Name → `refund_adjust_by_wallet`;留痕 3 —— ① service 裸 select 绕 Repository(平台级跨租户只读 job,plan §4.2 明文 + scan_balance_warnings 先例;铁律 2 管的是租户过滤查询,此处无租户过滤可下沉)② `_new_tenant_signal` 裸 dict(details_json 需 JSON 序列化,内联最诚实)③ run() ~165 行六件事(切片 02 加通知编排时再评估拆 `_build_report`);另 2 测试侧裁量留痕:seed helper 与 test_billing 近重复(仓库各文件自带 helper 惯例,rule of three 未到)/测试硬编码 action 串不 import 常量(独立真相源防同源漂移)。**Spec 8/8 AC 全落地**:逐项核验含 AC6 用例映射与 AC8 不越界(diff 仅 5 文件,scheduler_enabled 默认未动,零通知零前端零迁移);2 低危偏差 —— ① 残余差共用 cutoff 窗口 vs §4.7 字面「不需要缓冲」→ **plan §4.7 回写实施注记**(不共用则用例 5 的 −5min 在途事件在聚合层误报;「不需要缓冲」应读作「不需要第二个缓冲概念」;refund/adjust 带符号求理同理入档)② skip 锁只查最近一条记录的 as_of(force 回跑旧 as_of 后同日会再插一条)——plan §4.7 自身字面继承,留痕不改。

### 验证

定向 `pytest tests/test_billing_reconciliation.py -q` **11/11**;ruff check 全绿;全量 `pytest -ra --strict-markers` **1059 passed, 8 skipped**(基线 1048 + 新增 11 零回归;8 skipped = PG 门控),审查回写 rename 后复跑全量同值。

### 非末切片收尾

plan 切片 01 标题 ✅ + 8 AC 全勾 + §4.7 实施注记(check_plan_status_sync exit 0);feature_list **不动 evidence/status 保持 in_progress**(切片 02 收尾才动);**下一步:切片 02 超管 targeted 通知 + CONTEXT.md 术语 + feature 收尾(末切片)**。
**PR 证据(合并后补记)**:PR #171 已合并(merge `28a2034`,2026-08-17 07:27Z),CI 4/4 全绿(Migrations 43s / Backend 11m14s / E2E 1m56s / Frontend 28s);本地 main 快进同步,分支 `feat/billing-reconciliation-job-slice-01` 本地/远端已删,`gh api` 验证远端只剩 main。

## Session 219(2026-08-17):billing-reconciliation-job EP2 回环(grill → to-spec → 双轴自审 → to-tickets,规划会话)

**任务**:risk-hardening 系列第 4 条(pri 93,R3b 🔴)EP2——计费对账闭环。按 three-tier §3 单回环完成:`/grill-with-docs`(决策逐项拍板)→ `/to-spec`(v1)→ 对抗式自审(单模型双轴)→ `/to-tickets`(v2 双切片)。开工 `./init.sh` 冒烟绿。

### 决策(D1-D10;D1-D7 用户 AskUserQuestion 前两轮逐项拍板无默认采纳,D8-D10 次级工程形态问询未获回复按工程判断采纳并如实标注)

D1 口径=**双层:事件级+聚合**(NOT EXISTS 漏扣明细 + 残余差/钱包不变式兜底)/ D2 周期=**日报一天一次 09:30**(错开 09:00 余额扫描)/ D3 容忍=**精确 0 + 回看缓冲**(token 整数无 rounding,真实噪声只有窗口边界在途)/ D4 告警去重=**事件粒度首告 + 日报汇总提及存量**/ D5 通道=**三通道全上**(logger.error + SystemLog + 逐超管 targeted in-app 通知[平台级 tenant_id=NULL 通知 repo 等值匹配实际不可见,取证发现])/ D6 落表=**复用 SystemLog**(每 run 一条,details_json 明细;按需加表铁律不建 reconciliation_runs)/ D7 补扣=**只报不补**(总纲建议;定价快照漂移与租户关系需人判断,补扣走既有 adjust)/ D8 例行日报=SystemLog info 一条(「跑了没差额」vs「没跑」可区分)/ D9 缓冲=30min(在途窗口毫秒级,三个数量级冗余)/ D10 切片=2 片线性。

### 取证要点

资损口=`_record_usage` commit 事件(cost=NULL)与 `_charge_usage` 独立事务 charge 分离,失败仅 logger.exception+rollback → cost 留 NULL 无 consume 交易(billing_service docstring 宣称的 reconciliation job 不存在);consume 交易带 usage_event_id FK = 事件级 join 天然信号;钱包门(94 ✅)已止血新会话,本条兜底已发生/残余漏扣;`scan_balance_warnings`(test_notifications.py:365 session_factory 直调范式)是 job 可测先例;scheduler_enabled 默认 False(显式化归 R5);超管挂 Platform 租户(init_admin)→ targeted 通知可达。

### 产物

`harness/docs/plan-billing-reconciliation-job.md`(v2:§0 双轴自审回写 1🔴[as_of 日粒度幂等锁——重跑同窗口不插重复 run 记录]/7🟡/3🟢;§7 双切片:01 对账 job 核心[frontier,Blocked by 无]→ 02 超管通知+文档+feature 收尾[末切片];零迁移零前端零计费编排改动,新建 `app/services/billing_reconciliation_service.py` + scheduler 薄壳)+ feature_list.json `plan` 字段回指本文档(`status` 按用户指令保持 **not_started**,EP3 开工时与 plan 状态行同 commit 翻 in_progress——R1/wallet-gate 同款先例)+ sync-active 刷新 + `check_plan_status_sync` **exit 0**(68 plan 一致)+ progress.md 本条。

### 文档影响评估

改动 3 文档(feature_list.json[plan 字段+notes 一句] / progress.md / 新建 plan-billing-reconciliation-job.md);CONTEXT.md「对账」术语与项目指南注记归切片 02 实施时落;不动 AGENTS.md/README/项目指南(EP2 纯规划零代码)。

**下一步**:EP3 切片 01(frontier):`BillingReconciliationService.run(as_of, force)`(双层检出+30min 缓冲+as_of 幂等锁+首告去重)+ 每 run SystemLog + `scheduler.py` 薄壳 job + CronTrigger 09:30 注册 + `tests/test_billing_reconciliation.py` 用例 1-7(TDD 先红后绿;`./init.sh full` 基线 1048 零回归)。

## Session 218(2026-08-17):chat-stream-wallet-gate 切片 02(前端 402 充值引导,末切片 + feature 收官,EP3 实施会话)

**任务**:EP3 切片 02(末切片)— 前端 402 充值引导 + feature 收尾仪式。分支 `feat/chat-stream-wallet-gate-slice-02`(自 main afc11a9 新开),冒烟 `./init.sh` 绿。

### 实施(commit 798e0c9,TDD 先红后绿)

落地 3 改代码 + 2 新测试:① `frontend/src/api/endpoints/search.ts` 新增 `ChatInsufficientBalanceError`(镜像 `CompositeInsufficientBalanceError` 形状)+ `sendChatStream` 非 200 分支插 402 特判:fetch 路径手动 `resp.json()` 容错解析 body detail(非 JSON body 兜底文案与 `chat.py` L68 detail 逐字一致,半角逗号);401 特判逐字不动,其余状态码维持泛化「对话请求失败: N」。② `frontend/src/pages/chat/index.tsx`:`balanceError` state + catch `instanceof ChatInsufficientBalanceError` 分流——402 → 充值引导面板(Wallet icon + 标题「余额不足,无法发起对话」+ 后端 detail + 说明行 +「前往充值」→ /billing,镜像 composite AC4.8 含 text-foreground WCAG 处理)+ `localMessages` 快照回滚;其余错误维持 toast;双清除(handleSend 开头 + useEffect on selectedConversationId)。③ `composite-chat.ts` 头部「strict, unlike /chat/stream's … SSE error frame」过时注释改写为统一口径(两路径共享「402 = 余额不足」契约、各持错误类,零代码)。④ 测试 8 条:`chat-stream-402.test.ts` 单元 4(402 detail 透传 / 402 非 JSON 兜底 / 401 不回归[清 token + AUTH_EXPIRED_EVENT + 泛化]/ 500 泛化;stub 全局 fetch 手工最小 Response,async generator 首次 next() 才执行)+ `insufficient-balance-panel.test.tsx` 组件 4(面板渲染[CTA + detail,不弹 toast,不留假消息]/ 发送前清除 / 切换会话清除[Panel stub 按钮触发]/ 上一轮流式内容保留;mock search.ts 保留真实错误类,MemoryRouter 包裹)。红证:单元 4 条因类未定义红、组件 3 条因 402 走泛化 toast 红;实现后全绿。

### /code-review 双轴(general-purpose ×2 并行)→ 回写(commit b78c7bb)

**Standards 0🔴/3🟡**:修 1 —— 判断项② 402 回滚 `setLocalMessages(null)` 在「同会话先成功一轮、再被 402 拦」时误清上一轮流式内容(上一轮仅存 localMessages,messages 查询不按轮失效)→ 改快照恢复 `localBefore`(发送前引用)+ 补第 4 条组件用例锁「上一轮内容保留」;留痕 2 —— ① 充值面板 JSX + 错误类与 composite 各两份(rule of three:第三处消费前抽共享 `InsufficientBalancePanel`;plan §9「AC 只钉形态不钉」+ composite-chat.ts 注释双背书,当前有意决策)② 面板文案半角逗号忠于 plan AC 原文与后端 detail 逐字口径(composite 历史文案全角不追改)。**Spec 0 缺失 / 0 越界**:AC1-4 实测全满足(401 分支与 main 逐字一致、兜底文案字节级比对 identical、diff --stat 与 §11 文件清单精确匹配);`setLocalMessages` 回滚判「D4 合理推导非越界」并经建议入档 plan AC2 注记;composite-chat.ts 段级注释改写合规。

### 验证

前端 `npm test` **259/259**(基线 251 + 新增 8 零回归)+ `npm run build` 绿(1.33s)+ `npx oxlint` 0 warning 0 error(新测试 `rejectWith402` 的 require-yield 以带理由 disable 注释处置,仓库 secure-image.tsx 先例);`./init.sh full` **1048 passed, 8 skipped**(基线 1048 零回归,后端零改动)+ ruff 全绿。
...
**任务**:EP3 切片 01 — SSE 预检 402 + 共享 helper + 移除流内旧门 + 测试地基。分支 `feat/chat-stream-wallet-gate-slice-01`(自 main 5327e0f 新开),冒烟 `./init.sh` 绿(211)。开工翻页 commit bf501fd(feature_list not_started→in_progress + plan 状态行同 commit,check_plan_status_sync exit 0)。

### 实施(commit 21c1a7b,TDD 先红后绿)

落地 1 改代码 + 5 改测试 + 1 新 fixture:① `app/api/v1/chat.py` 新增 `_require_wallet_balance(db, user)` 模块级私有 helper(super_admin 直返;函数内延迟 import `BillingService` 沿本文件惯例;`has_balance` 为假 → `HTTPException(402, detail="token 余额不足,请联系总部充值")`;**无 try/except**,查询异常 500 与 composite 对齐,fail-open 随旧门移除)。② SSE endpoint:`_load_agent`(404)之后、`create_or_get` 之前调 helper —— 被拦请求收真 402 且不再白建会话/落用户消息;`event_source` 内旧钱包检查整段移除(platform_role 分支 + get_wallet/balance≤0 判断 + SSE error 帧 + fail-open except + 「missing wallet is intentionally NOT blocked」注释块;`import json` 保留,delta/error 帧仍用)。③ composite 内联「Wallet pre-check」三行替换为 helper 调用(行为零变化);相关注释/docstring 口径同步(节注释 + docstring「Stricter than /chat/stream's…」段改写为统一口径)。④ `tests/conftest.py` opt-in `funded_wallet` fixture(balance=1_000_000 + total_recharged 同额,docstring 注明镜像生产「出生即带钱包」+ opt-in 不击穿 no-wallet 语义测试)。⑤ `tests/test_billing.py` SSE 门矩阵 ①-⑧ 全落地(①无钱包 402[翻转旧 allowed]/②零余额 402 + Conversation/Message 计数均 0[翻转强化旧 blocked]/③负余额 404→402/④inactive 402/⑤super_admin 无钱包放行/⑥续问被 402 后该会话 messages 不变/⑦两路径 402 detail **逐字一致**[防御性 fail-loud stub]/⑧有余额 happy path 断言逐字保留);`_mock_chat` 加 conversation_id 参数。⑥ 19 个因改严而红的既有流式测试挂 funded_wallet(test_chat 11 / test_multi_agent 3 / test_usage_tracking 3 / test_customer_conversation 2,与 plan §4.6 预期清单完全一致);**唯一断言改动是收紧**:`test_chat_records_usage_on_message_and_ledger` 的 `ev.cost is None` → `== Decimal("0")`(旧断言锁「无钱包→charge no-op→cost 从未盖章」伪迹;真链路跑通后未配定价→cost 0 是 calc_cost 设计语义)。⑦ `tests/test_composite_chat.py` 两处「unlike /chat/stream's allow」过时注释同步。

### 验证

TDD 红证:改前矩阵 ①②③④⑥⑦ 红(assert 200 == 402,旧宽纵行为)、⑤⑧ 绿 → 改后全绿。定向 `pytest tests/test_billing.py tests/test_composite_chat.py -q` **48 passed**(composite 行为零变化);6 受影响文件 99 passed;**`./init.sh full` 1048 passed, 8 skipped**(基线 1043 + 净增 5 = 矩阵 8 接管旧 SSE 门 3;8 skipped = PG 门控),ruff 全绿零回归。

### /code-review 双轴(general-purpose ×2 并行)→ 回写(commit a655bbe)

**Standards 0 硬违规 / 3 判断项**:修 1 —— 矩阵 ① 未用 db_session/test_env 参数瘦身;留痕 2 —— funded_wallet 是第 3 份钱包 seeder(fixture 无法 import 测试模块私有 helper,conftest 是正确归属,聚合重构独立立项)/ ⑦ 的 `_no_stream` raise+yield 伪造 async generator 手法(注释声明、目的明确)。**Spec 8/8 AC 全满足 + 0 scope creep**:E2E/CLI 零影响锚点复核(diff 无 cli/frontend 改动);2 判断项 —— cost 断言重定基判「可接受,非放宽」(已回写 plan AC6 注记)/ test_composite_chat.py 注释同步判「同域注释不算越界」(Spec 轴发现的文件清单缺漏,已补记 plan 文件清单);1 措辞披露建议(cost 断言改动写进 commit body)已照做。

### 收尾

**PR #169 已合并(merge `99d1fd2`,2026-08-17 01:55Z)**:CI 4/4 绿(Migrations 50s / Backend 11m12s / E2E 2m24s / Frontend 33s);本地 main 快进同步 + 分支本地/远端已删。plan 切片 01 标题 ✅ + PR 证据 + 8 AC 全勾 + 完成证据节。**非末切片:feature_list 不动 evidence、status 保持 in_progress**(切片 02 收尾同 commit 翻 passing)。**下一步:EP3 切片 02(前端 402 充值引导 + feature 收尾,末切片)**:`search.ts` ChatInsufficientBalanceError + `chat/index.tsx` 充值面板 + 双清除 + vitest ≥2 条 + composite-chat.ts 注释修正 + feature 收尾仪式。**网络备注**:github.com:443 间歇不可达,git push/fetch 需 `-c http.version=HTTP/1.1` 且可能重试多次;`gh api`(api.github.com)全程稳定,远端验真以 gh api 为准。



## Session 216(2026-08-16):chat-stream-wallet-gate EP2 回环(grill → to-spec → 双轴审查 → to-tickets,规划会话)

**任务**:risk-hardening 系列第 3 条(pri 94,R3a 🔴)EP2——SSE 钱包门口径统一。按 three-tier §3 单回环完成:`/grill-with-docs`(8 决策全用户逐项拍板)→ `/to-spec`(v1)→ 对抗式审查(单模型双轴)→ `/to-tickets`(v2 双切片)。

**决策(D1-D8,AskUserQuestion 3 轮逐项拍板,无默认采纳)**:D1 预检时点=**建立连接前真 402**(`_load_agent` 404 后、`create_or_get` 前;附带:被拦不再白建会话/落用户消息)/ D2 口径=**逐字对齐 has_balance**(无钱包=拦·0/负/inactive=拦·>0 才放,资损口关死)/ D3 402 体=**复用 composite**(detail 同文案)/ D4 前端=**完整对齐 composite 充值引导 UI**(用户选了比「最小适配」推荐更大的面)/ D5 旧门=**移除流内检查+抽共享 helper**(两路径同一函数;fail-open 移除,查询异常 500 与 composite 对齐)/ D6 测试地基=**conftest opt-in funded_wallet fixture**(test_env 默认建钱包会击穿 composite no-wallet 测试,否决)/ D7 seam=**后端 HTTP 层 + 前端组件级**,helper 不单测 / D8 切片=**2 片线性**。

**取证要点**:SSE 现状=200+SSE 头已发后才在 event_source 查,无钱包永久放行(资损口本体),零余额被拦前已建会话+落用户消息;composite=has_balance 严格 402 干活前;测试爆炸半径=test_env 直建 Tenant 无钱包 → SSE 改严后 test_chat/test_multi_agent/test_usage_tracking/test_customer_conversation 流式测试需挂 fixture;E2E 用种子 super_admin(main-flow.spec.ts:15-17)两路径门都 bypass 零影响;CLI `stream_sse` ≥400→ApiError 透 detail(cli/client.py:89-95)零改动;前端 `sendChatStream` 现对 402 只会泛化「对话请求失败: 402」,composite 有 `CompositeInsufficientBalanceError`+充值面板先例(D4 拍板对齐)。

**产物**:`harness/docs/plan-chat-stream-wallet-gate.md`(v2:§0 双轴审查回写 Standards 0🔴/3🟡/3🟢 + Spec 0🔴/4🟡/1🟢,无 🔴——形态已被 composite 同文件验证 + 起草前完成全部代码取证;§6 双切片:01 后端统一钱包门[frontier,Blocked by 无]→ 02 前端 402 充值引导+feature 收尾[末切片];SSE 门测试矩阵 ①-⑧ 含两路径 402 detail 逐字一致断言)+ feature_list.json `plan` 字段回指本文档(`status` 按用户指令保持 **not_started**,EP3 开工时与 plan 状态行同 commit 翻 in_progress——R1 同款先例)+ sync-active 刷新 + check_plan_status_sync **exit 0**(67 plan 一致)。

**下一步**:EP3 切片 01(frontier):后端统一钱包门——`_require_wallet_balance` helper(super_admin 直返 + has_balance 为假 → 402「token 余额不足,请联系总部充值」,无 try/except)+ SSE 预检 402 + 移除 event_source 旧门(含 fail-open)+ composite 改调 helper(行为零变化)+ conftest funded_wallet + `./init.sh full` 零回归(基线 1043)。

## Session 215(2026-08-16):booking-toctou-guard 切片 02(服务层 400 映射,末切片 + feature 收官,EP3 实施会话)

**任务**:EP3 切片 02(末切片)— 服务层 exclusion violation 400 映射 + 判别单测 + PG 用例 ⑥ + 术语/文档同步 + feature 收尾仪式。分支 `feat/booking-toctou-guard-slice-02`(自 main 3057e1f 新开),冒烟 `./init.sh` 绿。

### 实施(commit ba98551)

落地 2 改代码 + 2 文档:① `app/services/booking_service.py`:`_EXCLUSION_CONFLICT_MESSAGE` 常量(D8 简文)+ `_map_exclusion_violation` 纯判别函数(`getattr(getattr(exc,'orig',None),'sqlstate',None) == '23P01'` → BizError,其余一律 None → re-raise 不吞)+ create(`repo.add` 捕获)/ update(`db.flush` 捕获)双路径 `mapped = ...; if mapped is None: raise; rollback; raise mapped from exc`。② 判别单测 3 用例挂 `tests/test_bookings_api.py` X 章节(23P01 → BizError / 23505 → None / 无 sqlstate → None,构造异常对象直测)。③ PG 用例 ⑥(`test_service_path_race_maps_to_bizerror`):conn1 未提交 active [10,12) + 另一 session `BookingService.create` [11,13)(super_admin 写路径绕 casbin;种子 users 行避 created_by FK 歧义;显式 import 5 个 model 模块注册 mapper——本文件不 import app 装配);`_await_blocked_insert` 轮询 pg_stat_activity(wait_event_type='Lock' AND query LIKE 'INSERT INTO bookings%')确认 INSERT 真阻塞后才 commit conn1(确定性握手替代 sleep 赌博),断言 BizError 精确消息(D8 简文无 id,与应用层带 id 文案可区分——不可能经应用层检查假通过)+ 事后恰一计数。④ CONTEXT.md 术语「占坑态(Slot-Holding States)」+ 项目指南数据库 ORM 文档「PG-only 约束:EXCLUDE 排他约束(bookings 范式)」段(DDL 形态 / 迁移持有模型不声明三理由[SQLite 编译不了 + alembic check 天然干净 + dialect guard] / PG 门控 vs SQLite 判别测试分工 / 服务层映射-同源守护-预检拒迁三条配套约定)。

### 验证

定向 `pytest tests/test_bookings_api.py` **98 passed**(95 基线 + 新增 3)/ PG 门控 **8/8**(docker PG 5433)/ `alembic upgrade head && alembic check` 干净 / `./init.sh full` exit 0 + 全量 **1043 passed, 8 skipped**(基线 1040 + 新增 3 零回归;8 skipped = PG 门控 SQLite 自动跳)/ 前端 `npm run build` 绿(零前端改动,2.42s)/ ruff 全绿。**踩坑记录**:① 误跑 `ruff format` 重排三个文件既有代码(仓库只强制 `ruff check`)——git checkout 还原后重做逻辑改动,diff 收敛 251+/3-;② PG 文件实例化 BookingService 首跑 mapper 解析失败(Device→DeviceModel 字符串目标无注册,SQLite 套件靠 conftest 全量 import 掩盖)——显式 import model 模块修复并注释声明;③ 后台任务管道 tail 截断丢失测试计数——证据用全量重跑补齐。

### /code-review 双轴(general-purpose ×2 并行)→ 回写(commit 1d874fc)

**Standards 0🔴/4🟡**:修 2 —— 判别单测 X1 消息断言改 import `_EXCLUSION_CONFLICT_MESSAGE` 单源(与 PG 用例 ⑥ 同口径)+ `_await_blocked_insert` 移文件顶部 helper 区(对齐既有布局);留痕 2 —— create/update 双捕获站点 8 行同形(plan §4.6 钦定「两处包捕获」+ 仓库 per-site inline try/except 惯例[Standards 轴核实 user_service/rbac_service 同款先例],不抽 context manager)/ X 章节字母跳号(mnemonic eXclusion,文件字母本就松散)。**Spec 6/6 代码 AC 全满足 + 0 scope creep + 0 实现错误**;对抗性确认:用例 ⑥ 经「握手 + 精确消息」双保险不可能经应用层检查假通过;1 措辞回写 —— plan §9 PG 测试 import 面风险行(用例 ⑥ 使文件 import 面扩至 service 层,原「只 import models/SQLAlchemy」失准)。
...
**任务**:EP3 切片 01 — DB 层结构性兜底:EXCLUDE 约束迁移 + 脏数据预检 + PG 门控测试 + CI。分支 `feat/booking-toctou-guard-slice-01`(自 main 3478061 新开),冒烟 `./init.sh` 绿(209)。

### 实施(commit 4ced028)

落地 1 新迁移 + 2 新测试 + 2 改:① 迁移 `2026_08_15_1430_9a8b7c6d5e4f_add_bookings_overlap_exclude.py`(接 b3f7a2c91d4e):预检拒迁两条并列(active 重叠对 a.id<b.id 去重 + 退化区间 end<=start,任一>0 → RuntimeError 带两数量与处置指引)+ 非 sqlite guard 内 `CREATE EXTENSION IF NOT EXISTS btree_gist` + `ADD CONSTRAINT excl_bookings_active_no_overlap`(device_id WITH = + tstzrange '[)' WITH &&,WHERE 谓词经 `_ACTIVE_STATES_SQL` 常量单源插值——迁移是冻结历史不 import 活代码,常量是 4 处 SQL 站点的单点)+ downgrade 镜像 guard 内 DROP CONSTRAINT IF EXISTS,btree_gist 保留不卸注释声明;② `tests/test_booking_overlap_pg.py`(PG 门控 skipif 非 PG):用例①-⑤ 全落地——并发恰一成功(create_task 延迟 commit 的阻塞-解锁确定性模式 + sqlstate 23P01 断言 + 事后恰一计数,finally 收口 committer task 防泄漏警告掩蔽真失败)/ back-to-back [10,12)+[12,14) 双成功 / cancelled+done+no_show 三态 parametrize 同窗重订 / UPDATE 改期撞重叠被拒(23P01)/ NULL device 双插不冲突 + 计数;marker 种子(tenants→device_models→devices 链,uuid hex 前缀)+ finally 按 FK 级联序清理;③ `tests/test_booking_overlap_migration_source.py`(SQLite 常驻防漂移):读迁移源码(剥 docstring 防引文误判)断言 `_ACTIVE_STATES_SQL` 与 `_ACTIVE_STATES` 逐字一致 + 4 SQL 站点全经 `{_ACTIVE_STATES_SQL}` 插值 + 无手写 `status IN (...)` 绕过;④ `app/models/booking.py` 仅 docstring(双防线 D7 + 考古 + 不进 `__table_args__` 两原因 + 同源互指),`__table_args__`/列零改动;⑤ `ci.yml` migrations job 在 upgrade+check 后加 step `pytest tests/test_booking_overlap_pg.py -q`(env 补 OPENAI_API_KEY/CORS_ORIGINS/JWT_SECRET/SALT_ROUNDS 对齐 backend job 双保险)。

### 验证(docker PG 本地 5433,注意不是 plan 原笔误的 5432)

TDD 红证(迁移未应用时跑 PG 测试:①④ DID NOT RAISE)/ `alembic upgrade head && alembic check` 干净(Spec 轴反射预判实证:背书索引 duplicates_constraint 剔除,env.py 零改动)/ PG 门控 7/7 / **拒迁实测序列**:downgrade → 手插 1 重叠对 + 1 退化区间 → upgrade exit 1 报文「found 1 overlapping active booking pair(s) / 1 degenerate active window(s)」版本未动 → 清理 → upgrade 成功约束回来 / downgrade 后 pg_catalog 确认约束消失(count=0)/ **`./init.sh full` 1040 passed**(基线 1038 + 新增 2 防漂移)+ 7 skipped(PG 门控自动跳过)零回归。**踩坑记录**:`docker exec` 不带 `-i` 时 heredoc 静默不进容器(psql 读空 stdin 退出 0),拒迁首轮「假成功」就是它——排脏数据必带 `-i` 并回查计数。

### /code-review 双轴(general-purpose ×2 并行)→ 修复(commit 50eaa67)

**Standards 0🔴 / 3🟡**:修 1 —— PG 测试 `_active_booking_count` 第四处手写状态清单 + 守护注释失实(防漂移测试只读迁移源码不覆盖它)→ 改 import `_ACTIVE_STATES` expanding bindparam 单源;留痕 2(防漂移正则防呆不防恶[换序/拼接可绕,AC 只要求字面量一致防线] / `_EXPECTED_CONSTANT_SITES=4` 是有意 tripwire 新增站点须同步改)。**Spec 7/7 无越界**(服务层映射/CONTEXT.md/数据库文档 = 切片 02 未触碰);2 小修:用例⑤补显式计数断言 + plan §4.6 本地复跑端口笔误 5432→5433 回写。

### 收尾

**PR #167 CI 4/4 绿**(Migrations 46s——含新 step「Postgres-gated booking-overlap tests: success」实证 / Backend 7m23s / E2E 1m45s / Frontend 34s)→ **merge 596dccd**(2026-08-15 15:37Z)+ 本地 main 同步 + 分支本地/远端已删。plan 切片 01 标题 ✅ + 7 AC 全勾 + 完成证据节;**非末切片:feature_list 不动 evidence、status 保持 in_progress**(切片 02 收尾同 commit 翻 passing);progress.md 本条 + 顶部 frontier → 切片 02。**文档影响评估**:① feature_list.json 本切片零改动(仅开工翻页)✅ / ② progress.md ✅ / ③ plan §6 ✅(勾选 + 证据 + §4.6 端口笔误回写)/ ④ CONTEXT.md 与项目指南**不受本切片影响**(「占坑态」术语与 PG-only 约束范式段是切片 02 的 AC,届时落)/ 不动 README。

## Session 213(2026-08-15):booking-toctou-guard EP2 回环(grill→to-spec→to-tickets,规划会话,零代码改动)

**任务**:risk-hardening 系列第 2 条(pri 95,R2 🔴)EP2——TOCTOU 竞态 DB 兜底烤清形态 + 落 PRD + 拆切片。冒烟 `./init.sh` 绿(209)。全程零代码改动(纯 harness 文档 + feature_list)。

### 考古(任务前置要求:为何当时刻意不加部分唯一索引)

出处 device-booking D8(用户拍板「不软删,只用 cancelled 态」)→ plan §4.4 连带「无 is_deleted 列,无部分唯一索引」;仓库部分唯一索引范式(devices serial / user_tenants SCD2 / wallet)全靠 `WHERE is_deleted = false` 排除软删行,bookings 无软删列范式无锚点。模型注释「overlap 是运行时业务规则非静态列不变式」对普通唯一索引成立(唯一索引挡不住区间部分重叠)。**结论:当时不是「考虑过 DB 兜底后拒绝」,TOCTOU 不在范围**。关键事实:对区间重叠做真·DB 层结构性拒绝的形态只有 PG EXCLUDE 约束(btree_gist + tstzrange &&);部分唯一索引只挡同一开始时刻(advisory lock 拒绝仍表现为应用层 400)。

### grill(8 项决策用户逐项拍板,AskUserQuestion 2 轮,无默认采纳——系列铁律)

D1 兜底形态 = **PG EXCLUDE 约束**(否决 advisory lock / 部分唯一索引)/ D2 排除态 = **对齐应用层 `_ACTIVE_STATES`**(pending/confirmed/in_service 占坑)/ D3 错误语义 = **同款 400**(延续 device-booking D1「400 非 409」)/ D4 测试落点 = **PG 门控测试 + CI Migrations job 加步**/ D5 存量脏数据 = **预检拒迁**(镜像 knowledge-foundation 先例)/ D6 覆盖面 = **create+update 全覆盖**(EXCLUDE 天然)/ D7 应用层检查 = **保留**(双防线)/ D8 兜底文案 = **简文不带 id**。

### to-spec v1 → 双轴审查 → v2

plan [harness/docs/plan-booking-toctou-guard.md](harness/docs/plan-booking-toctou-guard.md):**双切片**——01 DB 层结构性兜底(迁移:两条预检拒迁[重叠对+退化区间] + btree_gist + EXCLUDE,PG 门控测试 ①-⑤,CI wiring,同源防漂移测试)/ 02 服务层 400 映射(sqlstate 23P01 判别)+ 用例 ⑥(服务路径竞态全链:真 PG 上 BookingService.create 撞未提交行 → 断言 BizError 而非 IntegrityError)+ 术语/文档 + feature 收尾。**双轴审查 Standards 2🔴/2🟡/4🟢 + Spec 1🔴/4🟡/3🟢 全回写 §0**:🔴 downgrade 约束名笔误(双轴一致)+ feature_list plan 字段回填;Spec 轴实装版本源码级核实——psycopg 3.2.3 `sqlstate` 属性、**alembic check 反射预判方向反了**(SQLAlchemy 2.0.36 `duplicates_constraint` + alembic 1.14.0 `correct_for_autogen_constraints` 剔除约束背书索引 → check 天然干净,不需要 env.py 改动)、种子链必填列、预检退化区间缺口(补第二条计数)。

### 收尾

feature_list.json:plan 字段总纲 → 本文档 + notes 记 EP2 完成与 8 决策;**status 保持 not_started**(用户既定指令,EP3 开工与 plan 状态行同 commit 翻 in_progress);`sync-active-features.sh` 刷新(5 活跃)+ `check_plan_status_sync` **exit 0**(66 plan 一致);progress.md 顶部 frontier → EP3 切片 01 + 本条。plan 自检 4 项过(依赖图 01→02 无环 / 14 条 AC / 切片 01 frontier 无 blocker / 无悬空待定)。**文档影响评估**:① feature_list.json ✅ / ② progress.md ✅ / ③ plan-booking-toctou-guard.md 新建(v2)✅ / ④ 项目指南与 CONTEXT.md **不受影响**(EP3 切片 02 才落「占坑态」术语与数据库文档 PG-only 约束段,plan AC 已记);不动 README / 不动总纲(登记性质不变)。

## Session 212(2026-08-15):rate-limit-login-lockout 切片 03(TTL 收口 + 前端 429 事件桥,末切片 + feature 收官,EP3 实施会话)

**任务**:EP3 切片 03(末切片)— TTL 收口 + 前端 429 事件桥 + feature 收尾仪式。分支 `feat/rate-limit-login-lockout-slice-03`(自 main 9b6eefc 新开),冒烟 `./init.sh` 绿。

### 实施(commit aa8be67)

TDD 先红后绿(后端 2 用例 + 前端 3 用例)。落地 4 改 + 2 新:① `app/core/config.py` 两默认值 `access_token_ttl_minutes` 10080→480 / `session_ttl_hours` 168→8(注释声明存量 token 自然过期);② `.env.example` 两行显式 480/8 + 过渡注释(消除示例 60 与代码默认的静默分歧);③ `frontend/src/api/client.ts` 429 分支 → `RATE_LIMITED_EVENT="aap:rate-limited"` dispatchEvent(仿 AUTH_EXPIRED_EVENT 先例;不清 token 不跳登录不重试);④ `frontend/src/components/ui/rate-limited-toast.tsx`(新,~20 行返回 null 的 listener,挂 App.tsx ToastProvider 子树内,`t.error("请求过于频繁,请稍后再试")`,有意不做 dedup);⑤ `CONTEXT.md` 术语「临时锁定(Temporary Lockout)」vs 管理员锁定(按 plan §4.6 措辞,格式对齐判定链先例);⑥ 测试:`test_auth_local.py` +2(expires_in==28800 + UserSession expires_at∈(7h59m,8h],SQLite naive 走 `_as_utc`)+ `rate-limited-toast.test.tsx` +3(**stub adapter 塞真实 api 实例 defaults 驱动真拦截器链**,一用例覆盖 429→事件→toast 全链,另锁 429 不触发 AUTH_EXPIRED / 401 不回归且 token 不清;adapter 每用例还原)。

### 验证

定向后端 41 passed(test_auth_local 18 + test_auth + test_rate_limit 16)/ 全量后端 tests **1007 passed**(基线 1005 + 新增 2 零回归,另 cli 31 = 1038)/ `./init.sh full` exit 0 / alembic upgrade head + check 干净(docker PG,无新迁移)/ 前端 **npm test 251/251**(基线 248 + 新增 3)+ tsc -b 0 错 + build 绿 + oxlint 0/0 / grep `10080` 与 `\b168\b` 在 app/ 归零。

### /code-review 双轴(general-purpose ×2 并行)

Standards **0 硬违规**,6 判断项:① **配置指南表 `session_ttl_hours` 168 失真 + 前端 API 层文档缺 429 → 已修**(同 commit 同步 `项目指南/02-后端架构/02-配置与环境.md` 两行 + `03-前端架构/02-API层与请求拦截.md` 补 429 区别条目);② client.ts window 守卫与 401 同形(镜像先例保留);③ toast 文案组件/测试各一份(常规);④ RateLimitedToast 名含 Toast 渲染 null(JSDoc 自解释);⑤ 放 ui/ vs auth/(紧邻 toast.tsx 保留);⑥ 8h 双键 480min+8h(grill 拍板两独立键,测试锁对齐)。Spec **5/5 代码 AC 全满足 + 0 scope creep**(AC6 证据补记 plan;AC7 收尾仪式本节闭合)。

### feature 收尾仪式(three-tier §4 第 1-8 步,全 8 步闭环)

**CI 4/4 全绿**(PR #166,Frontend 28s / Migrations 48s / Backend 10m58s / E2E 1m55s)→ **PR #166 已合并(merge da02317,2026-08-15 13:53Z)**;本地 main 同步(da02317)+ 分支本地/远端已删(远端走 git push --delete,验证只剩 main)。① ./init.sh full + alembic + 前端三件套全绿 ✅ / ② feature_list.json `in_progress → passing`(EP2 笔误 not_started 已注记)+ evidence 4 条(切片 01/02/03 + 全量收官)✅ / ③ sync-active 刷新 ✅ / ④ plan 顶部状态行**同 commit** 翻 passing + 切片 03 标题 ✅ + 7 AC 全勾 + 完成证据节;`check_plan_status_sync` exit 0(65 plan 一致)✅ / ⑤ progress.md 本条 + 顶部 frontier → booking-toctou-guard(95)EP2 ✅ / ⑥ 文档影响评估:改动 3 文档(项目指南×2 + CONTEXT.md 术语新增)已同步 ✅ / ⑦ 依赖解锁扫描:无任何 feature depends_on → rate-limit-login-lockout,系列下一条 pri 95 booking-toctou-guard 成为新 frontier(需 EP2 回环)✅ / ⑧ 分支清理:本地 `-d` + 远端 `--delete` 均完成,本地/远端只剩 main ✅。

## Session 211(2026-08-14):rate-limit-login-lockout 切片 02(slowapi 全局限流,EP3 实施会话)

**任务**:EP3 切片 02 — slowapi 全局限流(单例装配 + 两档配额 + 路径短路豁免 + 429 + key_func + 测试)。分支 `feat/rate-limit-login-lockout-slice-02`(自 main 6dd5c54 新开),冒烟 `./init.sh` 绿(209)。非末切片:feature_list 保持 in_progress、plan 顶部状态行未动(切片 03 收尾才翻)。

### 实施前取证(写码前实证 slowapi 0.1.10 机制)

装包读源码 + 运行时实证:① `Limiter(enabled=)` 是可变属性,middleware/装饰器每请求检查;② `default_limits` callable 包装为 LimitGroup **每请求求值**(init 期 0 次调用,实测);③ `limit()` 装饰器有 `key_func` 覆写参数 + `override_defaults=True` 默认(严档真·覆盖默认档);④ **BaseHTTPMiddleware 版 SlowAPIMiddleware 的 `sync_check_limits` 对 async handler 会静默回退内置 `{"error"}` 体 → 自定义 429 handler 必须 sync**;⑤ `_inject_headers` 是 Limiter 方法(Retry-After/X-RateLimit-* 注入点);⑥ `reset()` 存在且 memory storage 可清。

### 实施(commit 5198b4f)

落地 7 改 + 2 新:① requirements.txt `slowapi==0.1.10` 精确 pin(附不设 RATELIMIT_* 键警示);② `app/core/rate_limit.py`(新):模块级单例 limiter(default_limits callable 读 settings + headers_enabled)+ `rate_limit_key`(验签 bearer → `user:{sub}`;无 token/验不过/`ahp_` PAT → IP;导入源 `app.core.security` 避 conftest 对 deps 的 mock)+ `RateLimitMiddleware(SlowAPIMiddleware)` 子类(豁免路径短路,不碰 `_exempt_routes` 私有 API);③ `app/core/security.py` 抽 `decode_token_sync` 同步核心(async wrapper 一行转发零行为变化,key_func 复用同一验签管线——裸解码可伪造 sub 绕过);④ config.py 四键 `rate_limit_enabled/default/login/exempt_paths` + frozenset 解析 property(镜像 cors_origins_list);⑤ main.py create_app:`app.state.limiter` + middleware + sync 429 handler(`{"detail"}` 体 + 复用 `limiter._inject_headers`);⑥ auth.py login 挂 `@limiter.limit(settings.rate_limit_login, key_func=get_remote_address)`;⑦ .env.example 四键 + 警示;⑧ conftest `RATE_LIMIT_ENABLED=false` + 会话级 `RATE_LIMIT_DEFAULT=3/minute`;⑨ `tests/test_rate_limit.py` 10 用例(严档第 6 次 429/enabled=false 不生效/默认档 429 且与登录档桶独立//health 20 连/key_func 四分支直测含伪造 sub 反绕过/两层共存:5 连败锁满+第 6 次正确密码 429)。

### /code-review 双轴(general-purpose ×2 并行)→ 修复(commit 84b30db)

Standards **0 硬违规**,4 判断项全留痕不改码(RATELIMIT_* 警示两处重复=受众不同各自规范位置;split/strip 形状 ×2=仓库 per-field property 惯例,第 3 处再抽;`decode_token` Middle Man=async 管线兼容有意为之;配额裸字符串=slowapi 原生接口)。Spec 6✅+3⚠️ 全闭合:**关键抓获(实证确认)**—— RateLimitMiddleware 原装配在 CORS 外层,默认档 429 在 middleware 内生成不回流 CORSMiddleware → 浏览器读不到 ACAO(严档 429 走 ExceptionMiddleware 反而带,两档不对称),切片 03 前端 toast 会失灵一半 → **修复:middleware 移到 CORS 之前(内层)**,E2E 复验两档 429 均 ACAO+Retry-After、CORS preflight 不再烧配额;AC5 豁免测试 parametrize 全 6 路径(+5 用例);conftest 注释纠偏(callable 每请求求值非 import 固化)。审查前另自查 CI 风险:E2E 单次登录远低于 5/min 严档,preflight 不烧配额,无风险。

### 验证

定向 39 passed(test_rate_limit 16 含成功登录回归 + auth_local/auth 23)/ 全量 **1036 passed**(tests 1005 + cli 31 = 基线 1020 + 新增 16 零回归)/ init.sh 冒烟 209 绿(修复后复跑)/ ruff 全绿 / **默认装配 E2E 实证**(pytest 外、真实默认配额):6 连打登录第 6 次 429+Retry-After:59+detail、/health 豁免、默认档 121 次 429、CORS 两档对称 ACAO、preflight 200 / **CI 4/4 全绿**(Frontend 27s / Migrations 44s / Backend 10m33s / E2E 2m5s)。

### E2E 排查插曲(commit 0599c5f)

CI E2E 两连红(5198b4f 与 84b30db 同签名:登录后停 /login)。本地起完整 E2E 栈复现拿到 traceback:**slowapi 装饰器端点返回 pydantic model 时,wrapper 成功路径往 `kwargs.get("response")` 注 X-RateLimit-* 头——login 缺 `response: Response` 参数 → 每次成功登录 500**。单测为何没拦:401/429 路径都在 `await func()` 前以异常短路,永不触达注入行;既有登录成功测试全跑在限流关闭下。修复 = login 签名加 `response: Response` + 补成功登录回归测试(200+token+X-RateLimit-Limit 三断言)。本地复现时聊天步另挂:worktree 起 **main 后端对照同挂** → 本地 DB 残留 LlmConfig 行致 LLM 回源非 mock 的环境因素(CI 全新 DB 无此问题,修复后 CI E2E 实测 2m5s 绿)。

### 收尾

plan 切片 02 标题 ✅(commits 5198b4f+84b30db+0599c5f,PR #165)+ 9 AC 全勾 + 完成证据节;**非末切片**:feature_list 不动、sync-active 不跑(无状态变化);progress.md 顶部 frontier → 切片 03(末切片)。**网络**:push 多次 443 超时,60s backoff 重试均通(Session 209/210 同款间歇)。

## Session 210(2026-08-14):rate-limit-login-lockout 切片 01(登录失败锁定,EP3 实施会话)

**任务**:EP3 切片 01 — 登录失败锁定(User 加列迁移 + UserRepository 写方法 + AuthService 判定 + 集成测试)。开工动作(用户既定指令):feature_list status not_started→in_progress + plan 顶部状态行**同 commit**翻页(`check_plan_status_sync` 65 plan 一致 exit 0,commit 0aa8220,规避 8a2c173/90da137 两次复发坑)+ sync-active 刷新。冒烟 `./init.sh` 绿(201)。

### 实施(commit b1c3268,分支 feat/rate-limit-login-lockout-slice-01,PR #164)

落地 5 改 + 1 新:① User 加 `failed_attempts`(int not null server_default "0")+ `locked_until`(timestamptz nullable),注释声明与 status="locked" 管理员永久锁独立;② 迁移 `b3f7a2c91d4e`(接 05fa069297cc,双库兼容纯列);③ UserRepository 三写方法(**方法内 commit**,失败路径 raise 前已持久化):`record_failed_attempt` 原子 `UPDATE ... SET failed_attempts = failed_attempts + 1 RETURNING`(杜绝并发 read-modify-write)/ `reset_failed_attempts`(清零+清锁)/ `set_locked_until`;④ AuthService.login:bcrypt verify 后、密码判定前 `_temporarily_locked` 判定 → 401 "account temporarily locked, try again later"(锁内正确密码也拒、失败不计数不续期);密码失败且本地账号 → 计数,达阈值 5 **先 reset 再 set**(reset 清 locked_until,按 AC 字面顺序会丢锁)锁 15 分钟 + SystemLog `action="login_locked"`(auth 模块 warning);成功登录 reset 清残留(在 memberships 判空后,无租户不清零);OIDC-only(password=None)不计数(锁对 Logto 路径无效,计数即 DoS 面);⑤ settings `login_lockout_threshold=5` / `login_lockout_minutes=15` + .env.example;⑥ tests/test_auth_local.py 扩 8 用例(5 连败锁+SystemLog 断言/过期恢复/锁内不计数不续期精确断言 locked_until 不变/成功清零再 4 败不锁/管理员锁不计数/不存在 10 连败/OIDC-only 连打/repo 原子直测 1→2)。

**实施期两个实证前提**(写码前跑脚本验证):SQLite 回读 `DateTime(timezone=True)` 丢 offset 成 naive → `_temporarily_locked` 归一化后再比较(PG aware 路径 no-op,测试 `_as_utc` 同款);aiosqlite(SQLite 3.50.4)`UPDATE ... RETURNING` 可用。

### 验证

定向 `pytest tests/test_auth_local.py tests/test_auth.py` **23 passed**(15 既有 + 8 新)/ 全量 **1020 passed**(基线 1012 零回归)/ `./init.sh` 冒烟 209 passed / alembic(docker desktop 本会话拉起)PG 实测 upgrade→downgrade→upgrade 回归 + `alembic check` 无 drift + psql 确认两列(`failed_attempts integer not null default 0` / `locked_until timestamp with time zone`)。

### /code-review 双轴(general-purpose ×2 并行)

Standards **0 硬违规** + 5 判断项全留痕(plan 完成证据节)不改码:repo 内 commit 偏离 flush 惯例(§4.6 对抗式审查后拍板的设计,保留)/ reset+set 可合一(plan 钦定三方法签名)/ 写方法不带 is_deleted(从 `update_last_login` 按主键写惯例)/ tz 归一化与测试同形两处(跨 app/test 边界各 3 行不提取)/ `int(or 0)` 兜底 vs `scalar_one()`(行被并发删时换优雅 401)。Spec **12/12 AC 全满足 + 无越界(未碰切片 02/03 域)+ reset→set 顺序判定正确**。

### 收尾

plan 切片 01 标题 ✅(commit b1c3268,PR #164)+ 12 AC 全勾 + 完成证据节;**非末切片**:feature_list 保持 in_progress、不动 evidence(切片 03 收尾才动);sync-active 刷新(active 视图 in_progress)。**CI 4/4 全绿**(Frontend 27s / Migrations[alembic PG]50s / Backend pytest+ruff 9m / E2E 1m50s)→ **PR #164 已合并(merge 888a70b,2026-08-14 13:55Z)**,本地 main 同步 + 分支本地/远端已删(远端走 gh api DELETE ref)。**网络**:push 443 超时多次、60s backoff 后第 5 次重试成功(收尾 docs commit);pull/远端删分支各超时 1 次后成功 —— Session 209 同款间歇故障,无残留债(本地 main = origin/main = 888a70b)。
