...
- **当前最高优先级未完成功能**: **`device-booking`(priority 63,设备预约订单 CRUD + 排期,系列 3/4,status=`in_progress`,EP3 进行中)**。设备功能系列进度:61(device-models-crud)✅ → 62(devices-crud-ui)✅ 全 passing → **63(device-booking `in_progress`,plan-device-booking.md 含 7 切片 + 12 决策)** → 64(device-poweron)。切片级状态真相源 = `harness/docs/plan-device-booking.md` 的 acceptance criteria checklist。切片进度:**切片 01(后端地基)✅ 已合并 PR #106 commit f2bfc93** → **切片 02(权限 seed + backfill)✅ 已合并 PR #107** → **切片 03(HQ 全景 + 排期聚合后端)✅ 已合并 PR #108 commit 2a69176** → **切片 04(customer own 端点 GET /me/bookings)✅ 已合并 PR #110 commit 24685a4(代码全绿 651 passed + ruff 0 + /code-review 0 阻塞)** → **切片 05(前端地基 types/endpoints/queries + 路由 + nav)代码已完成待合并** → 切片 06(前端门店端 StoreView)待做(frontier)。**下一步**:device-booking 切片 06(前端门店端 StoreView:列表 Table + filter chips + 排期网格 slot-box + CRUD Dialog),走 `/implement`(EP3,从 frontier 切片接)。注:切片 06 blocked by 05。
- **当前 blocker**: **网络超时(github.com:443 连不上)→ 切片 05 分支 `feat/bookings-slice-05-frontend-foundation`(commit `d18a60c`)本地已提交+全绿,但 push/PR/merge 暂时做不了。待网络恢复后:① push 该分支 ② 开 PR ③ 合并后回填 feature_list.json evidence + plan 标题加实际 PR# + 跑 sync-active-features.sh + 本文件 frontier 指向切片 06。代码本身已完成且验证通过(见下 EP3 断点),非代码阻塞。
- **EP3 断点(Session 135)**:device-booking 切片 05 代码已完成并本地提交(branch `feat/bookings-slice-05-frontend-foundation` commit `d18a60c`,基于 main 848f51e)。6 项 acceptance criteria 全勾(`plan-device-booking.md` 切片 05 标题已加 ✅ PR #111 占位,合并后改实际 PR#)。验证:./init.sh 全绿 651 passed(无后端改动,基线无回归)+ `cd frontend && npm run build` ✓ + `npx oxlint` 0 warnings/errors。/code-review 双轴 0 阻塞(Standards:1 个 JSDoc 一致性判断项已修 —— endpoints.ts bookings 段三个函数的 `/** */` 折叠进 section header,对齐 devices 段只 section-header 有注释的惯例;Spec:0 缺失,1 个低风险增量 `useBooking(id)` hook 对齐 useDevice/useCustomerProfile 范式)。改动文件:frontend/src/api/types.ts(+Booking/BookingCreate/BookingUpdate/BookingStatus/BookingHqRead/DeviceSchedule)、endpoints.ts(+fetchBookings/fetchBooking/createBooking/updateBooking/cancelBooking[POST /cancel]/fetchDeviceSchedule/fetchMyBookings)、queries.ts(+qk.bookings/deviceSchedule/myBookings + 7 hooks + BOOKING_WRITE_KEYS 失效集)、App.tsx(+lazy BookingsPage + /bookings Route 裸 ProtectedRoute)、nav-items.ts(+Calendar import + /bookings 项 menu:bookings + 业务管理 subgroup)、pages/bookings-page.tsx(新,最小占位 PageHeader)。无迁移、无后端改动。**续做待网络恢复:push → PR → merge → 回填 evidence/标题/sync-active/frontier→切片06**。网络若持续不通,切片 06 可在同分支或新分支本地继续(不依赖 05 合并,只依赖 05 代码已在本地)。
