...
- **当前最高优先级未完成功能**: **`devices-crud-ui`(priority 62,门店设备实例 CRUD + 绑定客户,Session 124 切片 01 后端地基已合并入 main)**。设备功能系列进度:61(device-models-crud)✅ → **62(devices-crud-ui,切片 01 ✅ 已合并 main PR #90,切片 02/03/04/05/06/07 待做)** → 63(device-booking)→ 64(device-poweron)。最新合并到 main:`devices-crud-ui 切片 01`(priority 62)✅ ship-it 流水线零修复合入 —— Device 表 + 5 端点 + model_id 完整性守卫 + 14 测试。
- **当前 blocker**: 无
...
> 用于在新对话中继续 devices-crud-ui 切片 03。复制下面 `---` 包裹的内容作为新对话的第一条消息。

---
/implement devices-crud-ui 切片 03:HQ 全景视图(后端,跨租户只读)

## 任务
实施 harness/docs/plan-devices-crud-ui.md「实施切片 → 切片 03」章节。
先读该文档的「v2 修订记录 🔴#3」「§6 API HQ 分流」「§8 测试 E 章节(super_admin 全景)」,
再读切片 03 的 acceptance criteria 逐项落地。

## 前置
切片 01 已合并入 main(Session 124,PR #90):Device ORM + alembic 迁移 + Repository/Service/API + 14 测试 A/B/C/D/G/H 全绿。
切片 02 已完成(Session 125):`DEFAULT_*_PERMS`/`DEFAULT_MENU_PERMS` 加 devices + `backfill_devices_perms_for_existing_tenants` 函数 + `scripts/backfill_devices_perms.py` + K1-K6 测试,600 passed baseline。
切到 main 的最新分支起点为切片 01 + 02 落地的代码。

## 开工流程(按 AGENTS.md,依次做,不要跳)
1. pwd 确认在仓库根目录(/Users/star/hugo/3-项目代码/project/ai-agent-platform)
2. 读 progress.md 恢复进度(尤其 Session 124 切片 01 + Session 125 切片 02 记录)
...
## 本切片要做什么(逐项)
1. `app/schemas/device.py` 加 `DeviceHqRead`:
   - 全景字段:`tenant_id`/`tenant_name`/`model_name`/`customer_name`(跨租户 viewer 用)
   - 继承 `DeviceRead` 的全部字段,加 4 个跨租户展示字段
2. `app/repositories/device.py` 加 `list_all_with_meta()` / `get_all_with_meta(device_id)`:
   - 用 `selectinload(Device.tenant)` / `selectinload(Device.model)` / `selectinload(Device.customer)` 防 N+1 + async session `MissingGreenlet`
   - **不复用** `customer.batch_tenant_info`(那是 customer 域耦合,只返 tenant_name,不返 model_name/customer_name)
   - 软删过滤(`is_deleted=False`)
3. `app/services/device_service.py`:
   - `list(actor_id, tenant_id, platform_role)` / `get(device_id, tenant_id, platform_role)` 加 `platform_role` 参数
   - 用 `is_cross_tenant_viewer(platform_role)` 分叉:跨租户返 `DeviceHqRead`(调 `list_all_with_meta`/`get_all_with_meta`),否则返 `DeviceRead` + 本租户(原切片 01 行为)
   - 跨租户 viewer 不调 `permission_service.require`(底层 `permission_service.check:103` 的 `platform_role == "hq_staff" and act == "read"` 特判 + `super_admin` bypass 已经放行)
4. `app/api/v1/devices.py`:
   - **`GET /` 和 `GET /{id}` 改为端点函数体内分流**(`if is_cross_tenant_viewer(user.platform_role): → DeviceHqRead 全景`;否则 `await permission_service.require(user.user_id, user.tenant_id, "devices", "read", platform_role=user.platform_role)` 走本租户)
   - **移除切片 01 临时用的 router-level `require_permission("devices","read")` 依赖**(否则 hq_staff 被直接 403)
   - POST/PUT/DELETE 保持 router-level `require_permission("devices","create/update/delete")`(写端点 hq_staff/super_admin 无 tenant role 时正常 403)
5. `tests/test_devices_api.py` 加 HQ 章节:
   - super_admin 拿全景字段(`tenant_name`/`model_name`/`customer_name`)且能读跨租户 device
   - hq_staff 拿全景字段 + 写端点(create/update/delete)返 403
   - 用 `super_admin_client` / `hq_staff_client` fixture(已存在)
   - 注意:每测试只用 ONE client fixture(避免 super_admin_client 污染共享 owner.platform_role,见 test_devices_api.py 顶部说明)
6. 文档影响评估:本切片暴露 customers-page 当前 HqView 对 hq_staff 不可见的**既存 bug**(WIP=1 不在本切片修),写入文档影响评估

## 边界(WIP=1,严格不做,做了就是越界)
- ❌ bind/unbind 端点(切片 04)
- ❌ 前端任何文件(切片 05-07,`DeviceHqRead` 类型留给切片 05 加 frontend type)
- ❌ 改动切片 01 章节 A/B/C/D/G/H 测试 + 切片 02 K 章节(只是 GET / 和 GET /{id} 守卫从 router-level 移到端点内,A/B/D/G/H 测试行为应不变,但 D 章节的 hq_staff 测试可能要补)
- ❌ 顺手"重构"或"清理"任何既存代码
- ❌ 修 customers-page HqView 对 hq_staff 不可见的既存 bug(留给后续 customer feature)

## 关键约束(项目铁律,违反即返工)
- HQ 读分流必须在**端点函数体内**,不能用 router-level `Depends(require_permission("devices","read"))` —— 那会直接 403 hq_staff。放行路径是 `permission_service.check:103` 的 `platform_role == "hq_staff" and act == "read"` 特判 + `super_admin` bypass,必须让代码走到那一层
- `list_all_with_meta` / `get_all_with_meta` **必须 selectin load** tenant/model/customer,否则 async session 触发 `MissingGreenlet`(SQLAlchemy 2.0 async 在 lazy load 时报错)
- 不复用 `customer.batch_tenant_info`(customer 域耦合,只返 tenant_name)
- 切片 01 GET / 对 hq_staff 是 403(预期,本切片改为 200 + 全景)

## 完成定义(对照切片 03 acceptance criteria 逐项打勾)
- `DeviceHqRead` schema 加全景字段(tenant_id/tenant_name/model_name/customer_name)
- `DeviceRepository.list_all_with_meta()` / `get_all_with_meta(device_id)` 实现,用 selectinload
- `DeviceService.list(...)` / `get(...)` 接 `platform_role` 参数,用 `is_cross_tenant_viewer` 分叉
- `GET /` 和 `GET /{id}` 改为端点内分流,移除 router-level read 守卫
- HQ 章节:super_admin 拿全景字段 + 读跨租户,hq_staff 拿全景字段 + 写端点 403
- `./init.sh` 全绿(ruff + pytest,新增 HQ 测试,零回归)

## 收尾(做完后必做)
1. 跑 ./scripts/sync-active-features.sh 刷新 active 视图(如果状态有变)
2. 更新 progress.md:标记切片 03 完成,下一步切片 04
3. 完成这个切片任务之后,给我下一个切片(04)的提示词,用于在新的对话当中
