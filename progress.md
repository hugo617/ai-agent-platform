...
- **当前最高优先级未完成功能**: **`knowledge-tiered-reader-ui` p88(知识库分级 Feature C:前端三栏可视化阅读页)— in_progress,EP3 切片 03 frontier(末切片)(plan=`harness/docs/plan-knowledge-tiered-reader-ui.md` 本 feature 专属;EP2 已完成 Session 197;切片 01 ✅ PR #155 merged commit 2df9e91;切片 02 ✅ commit 45151e3 待 PR;depends_on `knowledge-tiered-backend` p89 ✅ passing 已解锁)**。系列 4 feature:**p90 foundation ✅ passing 全 3 切片完成** / **p89 backend ✅ passing 全 4 切片完成** / **p88 reader-ui in_progress ← 当前 frontier**(切片 01+02 ✅ 完成;下一步 EP3 `/implement` 切片 03 末切片)/ p87 admin-ui not_started depends_on backend(已解锁,reader-ui 之后)。**reader-ui 切片进度**:01 地基(类型层+msw+barrel+DocumentList+scope-badge)✅ PR #155 / **02 左栏 CategoryTree + 右栏 MarkdownReader + 响应式折叠 ✅ commit 45151e3**(本次 Session 198)/ 03 CRUD Dialog 迁移 + RetrievalDebugCard 迁移 + 跨角色测试 + feature 收尾(末切片,待做)。**reader-ui EP2 产物(Session 197)**:plan-knowledge-tiered-reader-ui.md 落地,7 决策 —— G1 子组件自调 hook(右栏 reader 纯渲染例外)/ G2 三栏+迁移 CRUD+迁移 DebugCard(feature D 只做下发+Category 管理)/ G3 scope 实心 badge(destructive红/warning琉/success绿)与状态 dot badge 分层避混淆 / G4 lg 断点折叠(窄屏左栏 Sheet+中右纵叠)/ G5 搜索高亮 react-markdown components 自定义 render + `<mark>` + 跳转(不引新依赖)/ G6 类型层补齐纳入(DocumentRead 加 scope/group_id/category_id + KnowledgeCategoryRead + useDocuments 带参 + useKnowledgeCategories)/ G7 目录大纲前端正则提取标题。**关键决策**:引入 msw 补 API 集成测试层(偏离既有 mock-hook 范式,经用户确认,作为前端测试基建演进起点)。**切片 02 实现偏离(已文档化)**:react-markdown v10 的 `components.text` 不拦截 hast text 节点(实测 `<mark>` 不出现),G5 搜索高亮改在文本容器元素(p/li/strong/em/td/th/h2-h6/blockquote/a)render 里对直接字符串片段拆「前+`<mark>`+后」,行为等价 G5 意图,偏离仅实现层「text key→文本容器 key」(plan AC5 文案已修订反映 v10 现实)。**backend 全 4 切片**:`01 Category CRUD+scope 分级 ✅ commit e7297cd`(31 tests)+ `02 list+检索三路径+G1 bypass 接通 ✅ commit d21f8a9`(16 tests)+ `03 下发/撤回 API + distribute 权限码 ✅ commit dd43c0d`(35 tests,runtime seed 路径非 migration)+ `04 集成验证+feature 收尾 ✅ commit 962d7ff`(8 集成测试,987 passed 零回归)。**核心架构**:三级权限(super_admin / group_admin 派生身份=总部门店 owner/admin / 门店 owner/admin/member)+ 双维度分类(scope + category)+ 显式下发(引用模型,撤回软删)+ 三栏阅读。「门店=最小 OPC 产业单元」。**远端 main 真实 HEAD = `2df9e91`**(PR #155 merged,本地 main 已同步)。最近 passing:knowledge-tiered-backend(89)/knowledge-tiered-foundation(90)/member-service-direct-tests(85)/user-service-lookup-seam(84)/design-system-spacing-card-hierarchy(83,C)。
...
- **EP3 断点**: **knowledge-tiered-reader-ui EP3 切片 03(末切片)= frontier**(Blocked by 切片 01+02 ✅ 已完成)。切片 01 ✅ PR #155 merged commit 2df9e91(类型层 G6 + msw 基建 + barrel + DocumentList + scope-badge G3);切片 02 ✅ commit 45151e3 待 PR(左栏 CategoryTree + 右栏 MarkdownReader[G7 大纲 + G5 搜索高亮]+ 响应式 G4 Sheet 折叠)。切片 03 = 末切片(CRUD Dialog 迁移 + RetrievalDebugCard 迁移 + 跨角色测试 + 空态 + feature 收尾仪式 three-tier §4 第1-7步)。工作分支 `feat/knowledge-tiered-reader-ui-slice-02`(切片 02 commit 在此,切片 03 在同分支续或视 PR 情况新建)。**下一步**:新会话从切片 03 接 EP3(`/implement`)—— 切片 02 待开 PR + 合并;合并后切片 03 frontier 解锁(或切片 02 PR 评审期切片 03 可在新分支从 main 接)。
...
### 切片 02 — 左栏目录树 + 右栏 Markdown 阅读器 + 响应式折叠 ✅ commit 45151e3
