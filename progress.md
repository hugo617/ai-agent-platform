...
- **当前最高优先级未完成功能**: **bookings-shared-split**(priority 73,工程化,纯前端 locality 重构 — `bookings/shared.tsx` 19 导出符号按功能拆 4 文件 + shared 瘦身 + fmt 回源)。**EP2 回环已就绪**(2026-07-29,grill 7 决策 + 子智能体对抗式审查 v2 回炉 + 3 切片 tracer-bullet,plan `harness/docs/plan-shared-tsx-split.md`),depends_on=bookings-page-split ✅ 已 passing 依赖满足。**当前 frontier = 切片 1**(抽 `status-meta.ts` 4 符号 + `date-utils.ts` 5 符号,底座纯函数无 React 依赖)。composite-chat(priority 72)已于 2026-07-28 Session 155 全 4 切片收官 passing。详见 `harness/docs/plan-shared-tsx-split.md`。**最近 passing:composite-chat(2026-07-28 4 切片端到端全绿)**。
