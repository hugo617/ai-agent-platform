...
- **当前最高优先级未完成功能**: **`knowledge-tiered-admin-ui` p87(知识库分级 Feature D:前端分类管理 + 下发操作 UI + 后端接缝补齐)— in_progress,切片 01 ✅ + 02 ✅ + 03 ✅ + 04 ✅ + 05 ✅(代码 + 验证 + review 全完成),PR 待推(网络阻塞)**(depends_on `knowledge-tiered-backend` p89 ✅ passing 已解锁;`knowledge-tiered-reader-ui` p88 ✅ passing 已 merged PR #157 commit 15498e5)。系列 4 feature:**p90 foundation ✅ passing 全 3 切片完成** / **p89 backend ✅ passing 全 4 切片完成** / **p88 reader-ui ✅ passing 全 3 切片完成(PR #155/#156/#157 全 merged)** / **p87 admin-ui in_progress ← 末切片05 实施完成,仅剩 PR 推送 + merge + status flip**。**⚠️ 网络阻塞**:本会话沙箱 github.com:443 不通,本地 `feat/knowledge-tiered-admin-ui-slice-05` 分支 2 commit(4211025 实施 + b9a4aeb checklist)已就绪但 push 失败;且本地 main 已快进到 slice-04(4d4ee27),远端 origin/main 真实状态未联网确认(task 提示远端 06b7da2=切片04 merge)。**admin-ui 下一步(待网络恢复)**:① `git push -u origin feat/knowledge-tiered-admin-ui-slice-05`;② `gh pr create`(标题 feat(knowledge): admin-ui 切片 05 — reader-ui category 联动 B4 末切片);③ PR merged 后跑末切片收尾仪式:feature_list.json status in_progress→passing + evidence(切片01-05 全完成 + PR 链接日期)+ ./scripts/sync-active-features.sh + clean-state-checklist + 文档影响评估 + 系列收官(overview 加「系列状态:✅ 全部完成」)+ 分支清理(git checkout main && git pull --ff-only + git branch -d feat/...slice-04 feat/...slice-05)。**切片 04 已完成(Session 204,feat/knowledge-tiered-admin-ui-slice-04 分支)**:F6 category-manager.tsx(CategoryManager 按 scope 分组渲染 platform/group/store 三 Card 区块 + ScopeBadge + 区块内 sort_order 升序 + 每行 DropdownMenu 编辑/删除[canWrite 守卫];CategoryCreateDialog scope Select[getAvailableScopes 过滤]+ name + sort_order + scope 联动[group→group_id me.group_id / store→tenant_id me.tenant_id / platform→两者 null 隐藏]→ useCreateCategory;CategoryEditDialog 只改 name+sort_order[scope 只读 ScopeBadge 展示不可改,对齐 KnowledgeCategoryUpdate]→ useUpdateCategory;CategoryDeleteDialog 二次确认普通 Dialog[无 alert-dialog,镜像 distribution-list 范式]→ useDeleteCategory 软删)+ admin-panel 占位 CategoriesSubTabPlaceholder 删除改接 CategoryManager;12 测试(category-manager.test:scope 分组 2 + scope 过滤 2 + scope 联动 payload 3 + 编辑只改 name/sort_order 1 + 删除二次确认 2 + member 守卫 2)+ admin-panel.test 更新(占位→CategoryManager 断言 + stubBasics 补 category hook 兜底);npm test 244/244(232 baseline + 12 新)+ build + tsc + oxlint 0/0 零回归;/code-review 双轴:Standards 0 硬违规 + 2 判断项已修(① SCOPE_ORDER 从 category-manager.tsx + category-tree.tsx 两处本地声明提升到 scope-badge.tsx 共享[与 SCOPE_LABEL 先例一致消除重复];② 区块内按 sort_order 升序对齐 reader-ui category-tree 消歧义)/ Spec 6 AC 全绿 0 缺失 0 误 0 偏差。**切片 03 已完成(Session 203,feat/knowledge-tiered-admin-ui-slice-03 分支)**:F4 distribute-dialog(button-list 切「按门店」Checkbox 多选 /「按集团」Select 单选 XOR 构造 DistributeRequest;group_admin 锁本集团分店/集团、super 全平台;空选校验 + 成功 toast)+ F5 distribution-list-dialog(useDistributions 渲染已下发列表:门店名+distributed_at+生效绿/已撤回灰 + 每行撤回二次确认 → useRevokeDistribution)+ F5 admin-panel 文档表格行 DropdownMenu 加「下发」「管理下发」两入口(仅 isGroupAdmin||isSuperAdmin 可见 F7;owner 无操作列 + aria-label 无障碍);19 测试(distribute 9 + distribution-list 6 + admin-panel+4);npm test 232/232 + build + tsc + oxlint 0/0 零回归;/code-review 双轴 0 硬违规 + 1 Spec 判断项已修(distribution-list 改用 useGroups 解析门店名,移除冗余 useAllTenants 避免 group_admin 必 403 请求)。**实现偏离**:RadioGroup→button-list(无 radio-group.tsx 组件,同切片02 Tabs 先例)+ 二次确认用普通 Dialog(无 alert-dialog.tsx,镜像 document-list 范式),零新依赖,功能等价。**遗留 follow-up**:toast「含重新激活 M 条」需后端 KnowledgeDistributionRead 加 was_reactivated 字段(现 upsert 返回 row 重激活与新建不区分),已记入 plan,作为独立小切片(可并入切片05 收尾)。**切片 01+02 已 merged PR #158**(Session 203 Phase 1,merge commit 1594e40,1 次 CI 修红 plan 状态行 draft→in_progress)。**切片 02 已完成(Session 202,commit 2842dc3 + 7d8af12)**:F1 同页 button-list Tabs(阅读/管理,管理对 owner/admin 可见 member 隐藏 —— 镜像 settings-page 范式,非 plan 字面 shadcn Tabs,@radix-ui/react-tabs 未装用户确认)+ F2 admin-panel 子 Tabs(文档与发放/分类管理占位)+ F3 getAvailableScopes + isGroupAdmin helper(super→全/group_admin→group+store/owner→store/member→空,镜像后端 _resolve_create_target)+ document-form admin 创建表 Dialog(scope Select + group/tenant 联动 + category 下拉)+ 类型层(DocumentCreate 加 scope 等 + MeResponse 加 group_id+is_group_admin + 新 KnowledgeDistributionRead/DistributeRequest/KnowledgeCategoryCreate/Update)+ endpoints/hooks 扩(distribute/revoke/listDistributions + category CRUD);24 测试(admin-panel 11 + document-form 7 + knowledge-api msw +6);npm test 213/213 + build + tsc + oxlint 0/0 零回归;/code-review 双轴 0 硬违规 + 1 判断项已修(SCOPE_LABEL 导出共享消除第二份字典)。**切片 01 已完成(Session 201,commit 7bf8730)**:B1 MeResponse 加 group_id+is_group_admin(_build_me_response 反查 + super_admin 短路)+ B2 DocumentCreate 加 scope 字段 + _resolve_create_target 4 分支 scope↔角色校验(BizError→400 镜像 Category 范式)+ B3 list_distributions_for_source 三路径 + GET endpoint;17 测试(B1×3+B2×9+B3×5);./init.sh full 1004 passed 零回归;/code-review 双轴 0 硬违规。**admin-ui EP2 产物(Session 200)**:plan-knowledge-tiered-admin-ui.md 落地,13 决策(B1-B4 后端接缝 + F1-F7 前端形态 + T1-T2 测试切片)+ 5 切片线性(01 后端接缝 → 02 管理 tab 框架+创建表单 → 03 下发 Dialog+撤回 → 04 Category 管理 → 05 reader category 联动+收尾)。**范围扩展声明**:本 feature 含 3 个后端接缝补齐(对齐 reader-ui 含类型层补齐先例)—— ① /me 加 group_id+is_group_admin(前端判 group_admin 派生身份);② DocumentCreate 加 scope 等(上级创建 platform/group 文档);③ 新增 GET list distributions(下发 Dialog 显示已下发+撤回)。**核心架构**:三级权限(super_admin / group_admin 派生身份=总部门店 owner/admin / 门店 owner/admin/member)+ 双维度分类(scope + category)+ 显式下发(引用模型,撤回软删)+ 三栏阅读(reader-ui)+ 管理操作 UI(admin-ui)。「门店=最小 OPC 产业单元」。**远端 main 真实 HEAD = `1594e40`**(PR #158 admin-ui 切片01+02 已 merged,本地 main 已同步;此前 15498e5 = reader-ui 切片03)。最近 passing:knowledge-tiered-reader-ui(88)/knowledge-tiered-backend(89)/knowledge-tiered-foundation(90)/member-service-direct-tests(85)/user-service-lookup-seam(84)。
