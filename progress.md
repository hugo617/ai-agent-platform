...
> 用于在新对话中继续 devices-crud-ui 切片 05。复制下面 `---` 包裹的内容作为新对话的第一条消息。

---
/implement devices-crud-ui 切片 05:前端地基(types/endpoints/queries + isHQStaff + 路由)

## 任务
实施 harness/docs/plan-devices-crud-ui.md「实施切片 → 切片 05」章节 + 「前端实施」§1-§3 + §5 + §6。
先读该文档的「前端实施」§1(types.ts)/ §2(endpoints.ts)/ §3(queries.ts)/ §5(App.tsx 路由)/ §6(nav-items.ts)四节,
再读切片 05 的 acceptance criteria 逐项落地。

## 前置
切片 01 已合并入 main(Session 124,PR #90):Device ORM + alembic 迁移 + Repository/Service/API + 14 测试 A/B/C/D/G/H 全绿。
切片 02 已完成(Session 125):`DEFAULT_*_PERMS`/`DEFAULT_MENU_PERMS` 加 devices + `backfill_devices_perms_for_existing_tenants` 函数 + K1-K6 测试。
切片 03 已完成(Session 126):`DeviceHqRead` schema + Device model 3 个 relationship + `list_all_with_meta`/`get_all_with_meta`(selectinload)+ service `list`/`get` platform_role 分叉 + API GET 端点内分流 + E 章节 5 测试。
切片 04 已完成(Session 127):`DeviceBindRequest`/`DeviceBindResponse` schema + `service.bind`/`unbind`(幂等语义)+ `POST/DELETE /{id}/bind` 端点 + F 章节 8 测试,**613 passed baseline**。
切到 main 的最新分支起点为切片 01+02+03+04 落地的代码。

## 开工流程(按 AGENTS.md,依次做,不要跳)
1. pwd 确认在仓库根目录(/Users/star/hugo/3-项目代码/project/ai-agent-platform)
2. 读 progress.md 恢复进度(尤其 Session 124/125/126/127 切片 01-04 记录)
3. 读 feature_list.active.json 确认 devices-crud-ui 仍是最高优先级 not_started
4. git log --oneline -5 看最近发生了什么
5. ./init.sh 跑基础验证 —— 失败先修基础,不要在坏起点上叠新功能

## 本切片要做什么(逐项)
1. `frontend/src/api/types.ts` 加类型(全部从后端 schema 镜像):
   - `DeviceStatus = "active" | "maintenance" | "retired"`
   - `Device`:{id, tenant_id, model_id, serial_number, status, customer_id?: string|null, created_by?: string|null, created_at, updated_at}(对齐 `DeviceRead`)
   - `DeviceHqRead`:Device + {tenant_name?: string|null, model_name?: string|null, customer_name?: string|null}(对齐切片 03 `DeviceHqRead`)
   - `DeviceCreate`:{model_id, serial_number, status?, customer_id?}
   - `DeviceUpdate`:{model_id?, serial_number?, status?}(注意:customer_id 不在此 —— bind 走专用端点)
   - `DeviceBindRequest`:{customer_id: string}
   - `DeviceBindResponse`:{device_id, customer_id, already_bound: boolean}
   - `DeviceModelPublic`:{id, name, specs}(给入库下拉用,未来 device-models 管理页共用此类型;**只读子集**,不与未来 `DeviceModelRead` 冲突)
2. `frontend/src/api/endpoints.ts` 加 endpoints(参照既有 `fetchCustomers`/`createCustomer` 范式):
   - `fetchDevices()` → GET /api/v1/devices/
   - `fetchDevice(id)` → GET /api/v1/devices/{id}
   - `createDevice(payload)` → POST /api/v1/devices/ (201)
   - `updateDevice(id, payload)` → PUT /api/v1/devices/{id} (200)
   - `deleteDevice(id)` → DELETE /api/v1/devices/{id} (204)
   - `bindDeviceCustomer(id, customerId)` → POST /api/v1/devices/{id}/bind (200,返 DeviceBindResponse)
   - `unbindDeviceCustomer(id)` → DELETE /api/v1/devices/{id}/bind (204)
   - `fetchDeviceModels()` → GET /api/v1/device-models/(下拉用,返 DeviceModelPublic 列表)
3. `frontend/src/hooks/queries.ts` 加 query keys + hooks(参照既有 `useCustomers`/`useCreateCustomer` 范式):
   - `qk.devices` / `qk.deviceModels`
   - `useDevices()` / `useDevice(id)` / `useCreateDevice()` / `useUpdateDevice()` / `useDeleteDevice()` / `useBindDeviceCustomer()` / `useUnbindDeviceCustomer()` / `useDeviceModels()`(下拉用,enabled 守卫参照 `useAllTenants` —— 见 `frontend/src/pages/logs-page.tsx:89-94`)
4. `frontend/src/lib/permission.ts` 加 `isHQStaff(me)` helper(参照 `isSuperAdmin` L27-29 范式:`return me?.platform_role === "hq_staff"`)。**当前只有 `isSuperAdmin`,没有 `isHQStaff`** —— 必须新增。devices-page(切片 07)会用 `isSuperAdmin(me) || isHQStaff(me)`
5. `frontend/src/App.tsx`:`const DevicesPage = lazy(() => import("@/pages/devices-page").then(m => m.default))` + `<Route path="/devices" element={<DevicesPage/>}/>`(裸 ProtectedRoute,member 可读)。**注意:devices-page.tsx 本切片不存在**(UI 留给切片 06),所以 lazy import 会指向尚不存在的文件 —— 参照既有 customers-page 路由的写法,但要么(a)本切片同时建一个最小 placeholder devices-page.tsx(只渲染 `<div>devices</div>`),要么(b)把路由 + lazy 留到切片 06 一起加。**推荐 (a)**:建最小 placeholder 让路由可达,切片 06 替换为真实 UI。plan §5 只说"路由可达(空页即可)",所以 placeholder 符合 plan
6. `frontend/src/components/layout/nav-items.ts`:在「业务管理」subgroup 加 `{ to: "/devices", label: "设备", icon: <Monitor/>, menuCode: "menu:devices" }`(用 lucide-react `Monitor` icon)

## 边界(WIP=1,严格不做,做了就是越界)
- ❌ devices-page.tsx 的真实 UI(列表 Table / 入库 Dialog / 编辑 Dialog / 绑定 Dialog / HqView —— 全留给切片 06/07)。本切片若建 placeholder,只渲染最小占位(如 `<div>devices</div>` 或一个空 Table 骨架),不实现交互
- ❌ 改动后端任何文件(切片 01-04 已定型,schema/endpoint 契约不动)
- ❌ 修 customers-page.tsx 的 HqView 对 hq_staff 不可见既存 bug(留给后续 customer feature)
- ❌ 顺手重构/clean up 既存前端代码

## 关键约束(项目铁律,违反即返工)
- **类型严格镜像后端 schema**:`DeviceBindResponse.already_bound` 是 boolean(不是 string);`DeviceUpdate` 不含 customer_id(bind 走专用端点,plan §3 schema 注释明确);`DeviceHqRead` 的 `*_name` 是 `string | null`(不是 undefined,后端返 null)
- **`isHQStaff` 必须新增**:当前 `permission.ts:27` 只有 `isSuperAdmin`。不加这个 helper,切片 07 的 HqView 分叉 `isSuperAdmin(me) || isHQStaff(me)` 会报错。本切片把 helper 加上,切片 07 直接用
- **nav-items 的 menuCode 是 `menu:devices`**(不是 `devices`):参照既有 customers 的 `menu:customers` 范式。这个 code 对应切片 02 backfill 的 `menu:devices` 策略,member 无此菜单 code 时侧边栏不显示入口
- **useDeviceModels 的 enabled 守卫**:参照 `useAllTenants`(见 logs-page.tsx:89-94),只在需要时拉(如入库 Dialog 打开时),避免首屏全量拉型号。具体 enabled 条件本切片可宽松(如 `enabled: true`),切片 06 接入 Dialog 时再收紧
- **路由可达 = placeholder 即可**:plan §5 说"空页即可,UI 实现留给切片 06/07"。本切片若建 devices-page.tsx,最小占位就行,不要提前实现 Table/Dialog(那是切片 06 的职责,越界)

## 完成定义(对照切片 05 acceptance criteria 逐项打勾)
- `types.ts`:`Device`/`DeviceCreate`/`DeviceUpdate`/`DeviceBindRequest`/`DeviceBindResponse`/`DeviceHqRead`/`DeviceModelPublic` 全部定义
- `endpoints.ts`:`fetchDevices`/`fetchDevice`/`createDevice`/`updateDevice`/`deleteDevice`/`bindDeviceCustomer`/`unbindDeviceCustomer`/`fetchDeviceModels` 全部实现
- `queries.ts`:`qk.devices`/`qk.deviceModels` + 8 个 hooks 全部实现
- `permission.ts`:`isHQStaff(me)` helper 新增
- `App.tsx`:`/devices` 路由可达(placeholder devices-page.tsx)
- `nav-items.ts`:业务管理加「设备」菜单项(menuCode: menu:devices)
- `cd frontend && npm run build` 通过
- `cd frontend && npx oxlint` 0 error

## 收尾(做完后必做)
1. 更新 progress.md:标记切片 05 完成,下一步切片 06(前端 StoreView)
2. 完成这个切片任务之后,给我下一个切片(06)的提示词,用于在新的对话当中
