...
- **当前最高优先级未完成功能**: **union-cast-split**(priority 75,in_progress)。第 6 次巡检(2026-07-29 Session 163)Top recommendation —— 前端 5 个 role-branching hook 返回 union(`Device[]|DeviceHqRead[]` 等),窄化散落 4 view 靠 12 处 `as` 断言(微恶化,第5次 ~10→12)。grill 8 决策定方向:**拆 role-specific hook**(useDevices/useDevicesAll,union 在 hook 层消灭而非 view 边界)+ queryKey 共享 + All 后缀(随 useAllTenants 先例)+ 不提 ADR + ModelOption 投影 cast 不纳入(正交)。纯前端类型重构,零后端零 schema 零运行时行为变化,3 切片按 domain 分(bookings / devices / 收尾)。**plan**:`harness/docs/plan-union-cast.md` draft v1。**切片 01 ✅ commit abce938**(bookings domain)+ **切片 02 ✅ commit 待提交**(devices domain,本次 Session 158:消 8 处 A 类 device cast —— endpoints.ts 新增 `fetchDevicesAll`/`fetchDeviceModelsAll` + 窄化 `fetchDevices`→`Device[]`/`fetchDeviceModels`→`DeviceModelPublic[]`;queries.ts 新增 `useDevicesAll`/`useDeviceModelsAll`(共享 qk.devices/qk.deviceModels,D5);hq-view 改调 `useDevicesAll` 消 L163 + store-view 消 L152/355/365 三处 `as Device[]` + devices-page 双组件消 L192/422 + device-models-page 改调 `useDeviceModelsAll` 消 L149/216 + hq-view.test.tsx mock 改名 `useDevices`→`useDevicesAll`[D7];实施偏离 §4.5 改为窄化 `fetchDeviceModels` 镜像切片 1 对称范式,记入 plan)。下一步:EP3 `/implement` **切片 03 末切片**(A 类 cast 全仓审计 + B/C 类确认保留 + Note(candidate-8) 全清 + feature 收尾)。twospace-config(priority 74)全 3 切片已于 Session 162 收官 passing;bookings-shared-split(priority 73)已于 Session 158 收官 passing。
