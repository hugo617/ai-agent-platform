...
- **当前最高优先级未完成功能**: **`knowledge-tiered-foundation` p90(知识库分级 Feature A:数据模型 + 权限地基)— EP3 切片 02 ✅ 完成(Session 191,commit pending),frontier 推进到切片 03(集成验证+收尾,末切片,Blocked by 02 已解锁)**。plan=`harness/docs/plan-knowledge-tiered-foundation.md`(status=in_progress,WIP=1 frontier)。系列 4 feature(p90 foundation in_progress 切片01+02✅ / p89 backend not_started depends_on foundation / p88 reader-ui depends_on backend / p87 admin-ui depends_on backend)。EP1 总纲 12 决策(D1-D12)固化于 `plan-knowledge-tiered-overview.md`(Session 188)。EP2 grill 8 深化决策(E1-E8)固化于 foundation plan §4.5 + §4.7 check()签名适配 + §4.8 四 bypass 边界。**3 切片进度**:`01 数据模型地基 ✅ commit 4fb20b6`→`02 权限派生+自动化 ✅ commit pending`(is_group_admin 派生 helper + check() knowledge bypass + create_tenant 第7步单门店自成一集团,15 tests,90 smoke passed,双轴 review 2 发现全修)→`03 集成验证+收尾(末切片,frontier)`,严格串行无环。**切片 02 落地**:permission_service +`is_group_admin(db,user_id,group_id)` 模块级 async helper(查 group.headquarters_tenant_id → SCD2 current_role → role in owner/admin)+ 抽取 `_is_group_admin_of` 接受预取 group(check() 反推 group 后避免重查,code-review Feature Envy 修复)/ check()+require() 加 keyword-only `db: AsyncSession|None=None` 可选参数(60+ caller 不传→默认 None→bypass 不触发→走 casbin 零回归;AC6「签名不变」精神=caller 零改动,可选参数满足,ContextVar 方案评估后不采纳因偏离显式依赖风格)/ check() knowledge bypass 分支(is_platform_writer 后 casbin 前,obj=='knowledge' and db → list_for_tenant 反推 group → _is_group_admin_of → True;严格 scope knowledge D9,devices/bookings 走 casbin)/ require_permission 依赖注入 get_db 透传给 check() / tenant_service.create_tenant 第7步(直接 Group+Repository.add+attach,绕过 GroupService.create 因其内部 commit 破坏 AC8 同事务;wallet 后 commit 前)/ 15 tests(P is_group_admin 7 边界 + B check bypass 4 含 AC6 无db回归 + A 自动化 3 + AC8 事务回滚 1,monkeypatch attach 抛异常 + patch enforcer 验证全回滚)。**核心架构**:三级权限(super_admin / group_admin 派生身份=总部门店 owner/admin / 门店 owner/admin/member)+ 双维度分类(scope + category)+ 显式下发(引用模型,撤回软删)+ 三栏阅读。「门店=最小 OPC 产业单元」。第 9 次巡检候选 ①+④ 已收官(p84+p85)。**远端 main 真实 HEAD = `807f4df`(Session 187 经 `gh api` 验真;本地 HEAD `9707dc7` 内容等价 SHA 分叉属已知现象)。github.com:443 仍断,远端真相源以 `gh api` 为准**。最近 passing:member-service-direct-tests(85)/user-service-lookup-seam(84)/design-system-spacing-card-hierarchy(83,C)/design-system-color-sweep(82,B)/design-system-token-foundation(81,A)/queries-endpoints-domain-split(80)/customers-page-split(79)/devices-page-split(78)/perm-backfill-dedupe(77)/chat-page-split(76)。
