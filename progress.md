...
- **当前最高优先级未完成功能**: **twoscope-config**(priority 74,工程化,纯后端架构重构 — 抽 `TwoScopeRepository` 基类统一 3 个两级配置 repo 的读路径重复)。**EP2 回环已就绪**(2026-07-29 Session 159,第 5 次巡检 Top 候选 → grill 5 决策 + 子智能体对抗式审查 ×2[opus 真相核查 + 业务设计]→ 3 P0 回炉:决策3 加 is_active 死列→改钩子 / 删 slice3 ModelPricingService 空架子 / 补 ADR-0002 → v2 零 schema 零死列纯架构卫生)。plan `harness/docs/plan-twoscope-config.md`,3 切片 expand-contract。**当前 frontier = 切片 3(末切片)**(docstring 互指清理 + ADR-0002 + CONTEXT.md 指针 + feature passing)。切片 1 ✅ + 切片 2 ✅ 已完成(三 repo 全部改继承基类 + repo 契约测试覆盖三 repo 四态/三态)。bookings-shared-split(priority 73)已于 Session 158 全 3 切片收官 passing。详见 `harness/docs/plan-twoscope-config.md`。
- **bookings-shared-split ✅ passing(2026-07-29 Session 158 收尾,全 3 切片完成)**:bookings-page-split Session 139 留痕的 Divergent Change 收债(`bookings/shared.tsx` 19 导出符号混 5 类功能不相关内容,ScheduleGridCard + 私有 date helpers 只被 StoreView 消费却混共享文件)。**EP2 回环**(2026-07-29):`/grill-with-docs` 7 决策(D1 按功能拆 4 文件 + shared 瘦身保留 / D2 瘦身保留作真共享底座 / D3 不补单测沿用测试跟 view 走 / D4 纯 deep import 无新 barrel / D5 fmt+fromDatetimeLocalValue 回源 @/lib/format / D6 slotTone 归 schedule-grid-card 非 badges / D7 deviceNameOf 留 shared 避免孤儿)+ **子智能体对抗式审查 v2 回炉**(v1 发现 3 P0:export 19 非 17 / 消费者 5 非 6 / fmt+deviceNameOf+slotTone 三盲点 + 4 P1 全部修正)+ `/to-tickets` 3 切片 expand-contract。**EP3 切片链**:**切片 1 ✅ commit ae69dac**(抽 status-meta.ts 4 符号 + date-utils.ts 5 符号 + shared.tsx 改 import-then-export facade,expand 阶段,消费者零改动)/ **切片 2 ✅ commit a0075d4**(抽 filter.tsx 4 符号 + schedule-grid-card.tsx 3 符号 + 改 5 消费者纯 deep import[D4 无 barrel]+ fmt/fromDatetimeLocalValue 回源 @/lib/format[D5]+ shared.tsx 瘦身 290→38 行只剩 BookingStatusBadge+deviceNameOf[D2/D7],contract 阶段)/ **切片 3 ✅ 末切片**(收尾验证,无新源码):`./init.sh full` **828 passed** 零回归(277s)+ `npx vitest run` **65/65** 全绿(store-view 6 + my-bookings-view 6 + hq-view 13 + schedule-grid 11 + config-dialog 5 + format 15 + key-spec-rows 7 + queries-booking-config 2)+ `npm run build` 绿(bookings-page chunk 31.29 kB 与切片 2 一致)+ `npx tsc -b` exit 0 + `npx oxlint` **0 warning 0 error**(101 文件)。**ScheduleGridCard smell 消解确认**:grep ScheduleGridCard/ScheduleSlot/slotTone/dayLabel/hhmm/startOfToday 在 shared.tsx 全 exit 1(已全部迁出);schedule-grid-card.tsx 单消费者仅 store-view.tsx(deep import 无 barrel);零残留(无文件从 ./shared 抓已迁出符号)。**feature 收尾**:status in_progress → passing + evidence 4 条(切片 1/2/3 + 收尾条)+ sync-active 刷新(active 视图 0 活跃 + 5 最近 passing)+ plan draft v2 → passing(切片 3 标题 ✅ + 6 AC 全勾 + 头部 status passing)+ progress.md 顶部「最高优先级未完成」清空。**末切片仪式依赖解锁扫描(three-tier §4 第 7 步)**:无任何 feature depends_on 指向本 feature(纯重构 feature,无规划下游)→ 无需推进。**文档影响评估**:① feature_list.json ✅ / ② progress.md ✅ / ③ plan-shared-tsx-split.md draft v2 → passing;不动 README / 不动 `项目指南/`(纯前端 locality 重构,现有架构文档完全覆盖)。**最近 passing:bookings-shared-split(2026-07-29 全 3 切片端到端全绿)**。
...
- **当前 blocker**: 无
- **EP3 断点**: **twoscope-config 切片链 frontier = 切片 3(末切片,未启动)**(切片 1 ✅ commit 2ab3952 + 切片 2 ✅ Session 161,2026-07-29)。**切片 1 落地**:新建 `app/repositories/two_scope.py`(`TwoScopeRepository(BaseRepository[M])` 基类,含 `_active_filter` 钩子默认 None + `get_platform`/`get_for_tenant`;查询语义逐字对齐 llm 现状,逗号 where→链式 where 经审查确认 SQLAlchemy AND 等价产出同 SQL)+ `LlmConfigRepository` 改继承基类(`_active_filter = LlmConfig.is_active.is_(True)`,删自写两方法)+ 新建 `tests/test_two_scope_repo.py`(llm repo 四态契约:平台行/租户行/无行/is_active 过滤,态4 同时播 inactive 平台+租户行证两 scope 都过滤)。**切片 2 落地(migrate batch)**:`EmbeddingConfigRepository` 改继承基类(`_active_filter = EmbeddingConfig.is_active.is_(True)`,删自写两方法)+ `BookingConfigRepository` 改继承基类(`_active_filter = None`,BookingConfig 表无 is_active 列故不过滤,删自写两方法;查询语义逐字等价 — 旧 `.where(tenant_id.is_(None))` → 基类 `_active_filter=None` 不追加谓词,产出同 SQL)+ `tests/test_two_scope_repo.py` 补 embedding 四态 + booking 三态 + 额外 `test_booking_no_active_filter_all_rows_visible`(显式验证 None=不过滤契约,播平台+租户行证两行都返回)。**零行为变更**:service 层不动(_upsert/get_effective 各留)/ 零 schema / 不碰 ModelPricing(§11 边界)。**验证**:ruff clean + 新 repo 测试 12 用例全绿 + test_embedding_config + test_booking_config_api.py + test_llm_config 36 passed 零回归 + `./init.sh full` **840 passed**(基线 828 + 新增 12 repo 契约用例,零回归,252s)。**/code-review 双轴 GREEN**(子智能体 ×2 并行):Standards 0 硬违规(依赖单向 / 多租户隔离在 Repository 层 / 按符号引用全合规;两个 repo 正确镜像切片 1 llm 模式 + `_active_filter` 契约使用正确)+ Spec 5/5 AC 全满足(embedding is_active 过滤 + booking no-filter 语义逐字等价 SQL 确认);3 judgement-call smells(测试形状重复可 parametrize 但显式形式符合镜像约定 + 不过度设计,均非阻塞)。plan 同步:§6 切片 2 标题 ✅ + 5 条 AC 全勾。**切片链进度**:切片 1 ✅ commit 2ab3952(expand:基类 + llm 试点 + repo 契约测试四态)/ 切片 2 ✅(migrate:embedding + booking 改继承基类,booking 设 _active_filter=None 零 schema + 补 embedding 四态/booking 三态+no-filter 验证)/ 切片 3 ⬜(末切片:docstring 互指清理 + ADR-0002 + CONTEXT 指针 + feature passing;注:切片 2 因改父类已顺带删除 embedding/booking docstring 的 Mirrors 互指行,属被迫连带,切片 3 主要剩 llm docstring + ADR + CONTEXT)。bookings-shared-split(priority 73)全 3 切片已于 Session 158 收官 passing,作为前一 frontier 归档。
