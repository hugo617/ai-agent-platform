...
- **当前最高优先级未完成功能**: **`knowledge-tiered-reader-ui` p88(知识库分级 Feature C:前端三栏可视化阅读页)— in_progress,EP3 切片 01 frontier(plan=`harness/docs/plan-knowledge-tiered-reader-ui.md` 本 feature 专属;EP2 已完成 Session 197;depends_on `knowledge-tiered-backend` p89 ✅ passing 已解锁)**。系列 4 feature:**p90 foundation ✅ passing 全 3 切片完成** / **p89 backend ✅ passing 全 4 切片完成** / **p88 reader-ui in_progress ← 当前 frontier**(EP2 完成:grill 7 决策 G1-G7 + to-spec + to-tickets 3 切片;下一步 EP3 `/implement` 切片 01)/ p87 admin-ui not_started depends_on backend(已解锁,reader-ui 之后)。**reader-ui EP2 产物(Session 197)**:plan-knowledge-tiered-reader-ui.md 落地,7 决策 —— G1 子组件自调 hook(右栏 reader 纯渲染例外)/ G2 三栏+迁移 CRUD+迁移 DebugCard(feature D 只做下发+Category 管理)/ G3 scope 实心 badge(destructive红/warning琉/success绿)与状态 dot badge 分层避混淆 / G4 lg 断点折叠(窄屏左栏 Sheet+中右纵叠)/ G5 搜索高亮 react-markdown components 自定义 render + `<mark>` + 跳转(不引新依赖)/ G6 类型层补齐纳入(DocumentRead 加 scope/group_id/category_id + KnowledgeCategoryRead + useDocuments 带参 + useKnowledgeCategories)/ G7 目录大纲前端正则提取标题。3 切片线性:01 地基(类型层+msw 基建+barrel+DocumentList+scope-badge)→ 02 左栏树+右栏阅读器+响应式 → 03 CRUD+调试页迁移+收尾。**关键决策**:引入 msw 补 API 集成测试层(偏离既有 mock-hook 范式,经用户确认,作为前端测试基建演进起点)。**backend 全 4 切片**:`01 Category CRUD+scope 分级 ✅ commit e7297cd`(31 tests)+ `02 list+检索三路径+G1 bypass 接通 ✅ commit d21f8a9`(16 tests)+ `03 下发/撤回 API + distribute 权限码 ✅ commit dd43c0d`(35 tests,runtime seed 路径非 migration)+ `04 集成验证+feature 收尾 ✅ commit 962d7ff`(8 集成测试,987 passed 零回归)。**核心架构**:三级权限(super_admin / group_admin 派生身份=总部门店 owner/admin / 门店 owner/admin/member)+ 双维度分类(scope + category)+ 显式下发(引用模型,撤回软删)+ 三栏阅读。「门店=最小 OPC 产业单元」。**远端 main 真实 HEAD = `1d6dca7`**(PR #154 merged,本地 main 已同步 byte-for-byte 一致)。最近 passing:knowledge-tiered-backend(89)/knowledge-tiered-foundation(90)/member-service-direct-tests(85)/user-service-lookup-seam(84)/design-system-spacing-card-hierarchy(83,C)。
...
- **EP3 断点**: **knowledge-tiered-reader-ui EP3 切片 01 = frontier**(Blocked by 无,可立即 `/implement`)。EP2 回环 ✅ 完成(Session 197,grill 7 决策 G1-G7 + to-spec plan-knowledge-tiered-reader-ui.md + to-tickets 3 切片 + plan 自检 4 项通过)。3 切片线性:01 地基(类型层补齐 G6 + msw 基建 + barrel + DocumentList + scope-badge G3)→ 02 左栏 CategoryTree + 右栏 MarkdownReader(G7 大纲 + G5 搜索高亮)+ 响应式 G4 → 03 CRUD Dialog + 检索调试页迁移 + 跨角色测试 + feature 收尾。feature_list.json status `not_started → in_progress` + plan 字段指向 plan-knowledge-tiered-reader-ui.md + sync-active 刷新。**下一步**:新会话从切片 01 接 EP3(`/implement`)。分支 `feat/knowledge-tiered-backend-slice-02` PR #154 已 merged 进 main + §8 分支清理完成(本地+远端只剩 main);reader-ui 尚未开工作分支,EP3 切片 01 开工时新建。
