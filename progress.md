...
- **当前最高优先级未完成功能**: **bookings-shared-split**(priority 73,工程化,纯前端 locality 重构 — `bookings/shared.tsx` 19 导出符号按功能拆 4 文件 + shared 瘦身 + fmt 回源)。**EP2 回环已就绪**(2026-07-29,grill 7 决策 + 子智能体对抗式审查 v2 回炉 + 3 切片 tracer-bullet,plan `harness/docs/plan-shared-tsx-split.md`),depends_on=bookings-page-split ✅ 已 passing 依赖满足。**当前 frontier = 切片 2**(切片 1 ✅ 已完成 commit ae69dac on `feat/bookings-shared-split`:抽 `status-meta.ts` 4 符号 + `date-utils.ts` 5 符号 + shared.tsx 改 import-then-export facade,expand 阶段完成,消费者零改动)。切片 2 = 抽 `filter.ts`(4 符号)+ `schedule-grid-card.tsx`(ScheduleGridCard/ScheduleSlot/slotTone)+ 改 5 消费者 deep import + shared 瘦身到只留 BookingStatusBadge + deviceNameOf + D5 fmt/fromDatetimeLocalValue 回源 `@/lib/format`。composite-chat(priority 72)已于 2026-07-28 Session 155 全 4 切片收官 passing。详见 `harness/docs/plan-shared-tsx-split.md`。**最近 passing:composite-chat(2026-07-28 4 切片端到端全绿)**。
...
- **booking-schedule-grid 切片 04a ✅(2026-07-26,非末切片)**:ScheduleGrid 设备×时间核心网格组件(纯展示 + onSlotClick 回调,对齐 demo D0)。详见 plan §6 切片 04a 章节。
- **booking-schedule-grid 切片 03 ✅(2026-07-26,非末切片)**:前端 API 层 + 配置 Dialog(BookingConfigDialog 两栏/单栏 + 5 hooks + BOOKING_CONFIG_WRITE_KEYS)。详见 plan §6 切片 03 章节。
- **booking-schedule-grid 切片 02 ✅(2026-07-26,非末切片)**:后端按天查询端点 GET /bookings/schedule-grid + 复合索引 idx_bookings_tenant_schedule。详见 plan §6 切片 02 章节。
- **booking-schedule-grid 切片 01 ✅(2026-07-26,非末切片)**:后端 `booking_configs` 表 + 两级配置 API(frontier)。落地 7 新文件 + 3 改文件:model(`app/models/booking_config.py` 照抄 model_pricing 两级范式,tenant_id 可空 FK CASCADE,repo 继承 BaseRepository 非 TenantScopedRepository,无 DB 唯一约束对齐 D2)+ schema(`app/schemas/booking_config.py` Read/Upsert/Effective 三 DTO,duration Integer Field(ge=1)任意值非枚举对齐 D3,window HH:MM regex 校验)+ repo(`app/repositories/booking_config.py` get_platform/get_for_tenant 手写 where)+ service(`app/services/booking_config_service.py` get_effective 三级 fallback(tenant→platform→硬编码 45/08:00/22:00)+ upsert + LoggingService.record 审计 module="booking_config")+ api(`app/api/v1/booking_config.py` 5 端点,prefix=/bookings/config tags=bookings:platform GET/PUT require_super_admin / tenant GET/PUT require_permission settings:read/update + _ensure_tenant_access 跨租户 403 / effective query param 平台角色必带门店角色禁带)+ main.py 注册 router(对齐 tenant_config 范式)+ alembic 迁移(`2026_07_26_1000_a1b2c3d4e7g0`,新表 + seed 平台默认行 idempotent WHERE NOT EXISTS + server_default text("45"),无 DB 唯一约束)+ tests(`test_booking_config_api.py` 19 用例:P 平台 CRUD super_admin 200/其他 403 / T 租户 CRUD owner+admin+super_admin 200 member 403 跨租户 403 / E 三级 fallback 三态各测一次 / X effective 跨租户守卫 / V duration 任意正整数+非正/畸形 window 422 / A 审计 patch.object spy create→old_values=None update→old_values 非空)+ env.py/conftest.py 加 booking_config model import。**验证**:ruff clean + 全量 pytest 769 passed(基线 750+19)+ alembic upgrade head && alembic check 无 drift(Postgres aap_migcheck2)+ seed 平台行已验证(tenant_id=NULL/duration=45/window 08:00-22:00)+ grep 验证无 unique/uq_ 约束。**/code-review 双轴**:Standards 0 硬违规(修 2 判断项:schema docstring "All fields optional" 误导改"all required" + service _upsert 重复 message_scope 表达式提取一次);Spec 12/12 AC 全过(修 1 C2:migration default_duration_minutes 补 server_default text("45") + model 同步 server_default 对齐 §4.4 row 2 双 default 范式)。**非末切片,不做 feature 收尾仪式**(不刷 feature_list.json evidence/不 sync-active)。下一步:切片 02(按天查询端点 + 复合索引,Blocked by 切片 01 已解锁)。
- **booking-state-cancel ✅ passing(2026-07-25 Session 147 收尾,单切片即末切片)**:巡检候选 2([codebase-health-log.md] 2026-07-25 Strong,后端 deep module 收尾债)。**问题**:`booking_state.transition()` 管 start/end/no_show 三动作 6 边,但 cancel 判断独立内联在 `booking_service.cancel()`(if status==cancelled idempotent / elif !=pending raise BizError 400 / else flip),状态图劈两半,新人改状态翻两文件,且 cancel 用 BizError 而其他三动作用 InvalidTransition 错误语言不统一。**EP2 回环(Session 143)**:`/grill-with-docs` 6 决策 → `/to-spec` 落 plan-booking-state-cancel.md → `/to-tickets` 单切片(§6,16 条 AC)。**EP3 切片 01(Session 147,单切片 = 末切片)**:落地 `app/services/booking_state.py`(ACTIONS 加 'cancel' 4 元素 + _TRANSITIONS 加 ('pending','cancel'):'cancelled' 7 条边 + transition() 函数体零改动仍是 try/KeyError→InvalidTransition + module docstring 同步 3→4 动作 / 6→7 边 + 双层语义注释段)+ `app/services/booking_service.py`(cancel() 中间 5 行 if/elif/else + raise BizError + 直接赋值塌缩为 2 行:`if status=='cancelled': return True` 早退保留 D1/D2 + `booking.status = booking_transition(booking.status,'cancel')`;保留 require→_get_live_booking 顺序 D3 不对齐;docstring 同步)+ `tests/test_booking_state.py`(_LEGAL_EDGES 加 cancel 边 7 条 + test_legal_edges_count_is_six→is_seven 断言 7 + test_illegal_pairs_count_is_twelve→is_seventeen 断言 17(6×4−7)+ 新增 test_cancel_only_from_pending parametrize 5 非 pending 态 × cancel → InvalidTransition + module docstring + 测试旁双层语义注释段)。**零行为变更硬约束守住**:cancel 契约保留(204/idempotent)/ update 守 pending(D10 _MUTABLE_STATUSES 未碰)/ API 契约不变(POST /bookings/{id}/cancel 返回值/状态码/idempotency 全保留)/ 无 schema/迁移/前端改动。唯一文字变化:非 pending 非 cancelled 态 cancel 错误从 BizError("仅 pending…") 升级为 InvalidTransition("非法状态跳转…"),状态码 400 不变,异常类更精确(D4)。**feature 收尾**:status `in_progress → passing` + evidence 6 条 + sync-active 刷新(active 视图 0 活跃 + 5 最近 passing)+ progress.md 更新「最高优先级未完成」→ 无。文档影响评估:① feature_list.json ✅;② progress.md ✅;③ plan status `draft v1 → passing` + 切片 01 标题 ✅ PR #127 + 16 AC 全勾;④ `项目指南/` 不需补(纯重构,行为零变更,现有状态机/权限/多租户文档完全覆盖);不动 README。**末切片仪式依赖解锁扫描(three-tier §4 第 7 步)**:无任何 feature `depends_on` 指向 booking-state-cancel(巡检候选收尾债,无规划中下游)→ 无需推进。验证:`./init.sh` 全绿 750 passed(基线 739+11:test_booking_state 32 + test_bookings_api 90)+ ruff 0。/code-review 双轴:Standards 0 硬违反(2 判断项均文档化标准覆盖:D5 强制的双层语义注释重复 + frozenset 非 Enum 文档化);Spec 16 AC 全满足(1 out-of-scope drift 提示:errors.py docstring 仍说 6 边,不在本切片 §4.1 的 2 文件清单内,留 doc-sweep)。
- **platform-cross-tenant-write ✅ passing(2026-07-25 Session 146 收尾,全 5 切片完成)**:设备/预约功能系列(61-66)收官后的平台角色权限升格需求。**问题**:平台角色(super_admin + hq_staff)在 devices+bookings 是「跨店只读」(CROSS_TENANT_VIEWER_ROLES + permission_service.check read short-circuit);router 层 super_admin 已全 bypass,但 service body 有 owner-only/customer-only/hq_staff→403 硬分支,平台角色进了这些分支会被一并拒(无 tenant_id/不是 owner/不是 customer)。**EP2 回环(Session 144)**:`/grill-with-docs` 6 决策 → `/to-spec` 落 plan-platform-cross-tenant-write.md → `/to-tickets` 5 切片(§6,51 条 AC)。**EP3 切片 01-04(Session 145)**:4 个 feat commit + 2 个 docs commit 合并为 **PR #124**(squash,mergeCommit `c5bf99c`,CI 4/4 绿 Backend 7m58s / E2E 1m50s / Frontend 31s / Migrations 46s)。落地:① 共享鉴权基础设施(新文件 `app/services/_tenant_target.py` 导出 `resolve_target_tenant` 纯函数 + `is_platform_writer` helper + permission_service.check 的 platform_writer bypass for obj∈{devices,bookings});② devices 全 5 写动作(create/update/delete/bind/unbind)调 `resolve_target_tenant` + `elif is_platform_writer` 分支;③ bookings 全 6 写动作(create/update/cancel/start/end/no_show,切片 03 并入切片 02 —— act 白名单无法拆 + 防 hq_staff 静默写错店)同款改造,`_TRANSITIONS` 表零改动;④ 前端 HqView(devices-page + bookings/hq-view)加目标门店下拉 + 行内写控件 + `invalidateQueries` cache 保险,新文件 `bookings/shared-dialog.tsx`(从 store-view 抽出 5 Dialog + BookingRowMenu 接 tenantId prop),store-view 重构为调共享 Dialog(闭包模式 → store-view.test.tsx 断言零修改)。切片 03 act 拆分失败见 plan §4.5.4a 补丁 5。**EP3 切片 05(Session 146,末切片)**:`tests/test_hq_platform_role.py` +~110 行 —— 模块 docstring 重写(hq_staff 从「HQ read-only cross-tenant viewer」→「HQ cross-tenant viewer + writer」,反映角色契约变化)+ 新增「hq_staff cross-tenant WRITES」章节 2 正向写测(`test_hq_staff_writes_device_cross_tenant` POST /devices/ {tenant_id}→201 + DB 物理验证行落目标租户 / `test_hq_staff_writes_booking_cross_tenant` POST /bookings/ {tenant_id}→201 + DB 物理验证)+ 本地 `_seed_target_tenant` helper(tnt-hqw-+24hex,刻意本地化不共享)+ customers/groups 403 保留章节标题微调(outside platform-writer scope)。旧 `test_hq_staff_writes_are_403` 已在切片 01 改写(补丁 4),切片 05 不再碰。**feature 收尾**:status `in_progress → passing` + evidence 5 条(切片 01/02(+03)/04/05 + 收尾条)+ sync-active 刷新(active 视图 1 活跃 booking-state-cancel + 5 最近 passing)+ progress.md 更新「最高优先级未完成」→ booking-state-cancel。文档影响评估(plan §6 切片 05 AC 第 7 条):① feature_list.json ✅;② progress.md ✅;③ RBAC 文档不需补(`is_platform_writer` 与 `is_cross_tenant_viewer` 同范式,文档已覆盖平台角色 read bypass,write bypass 同理);④ plan draft v1 → passing(切片 05 标题 ✅ + 7 AC 全勾);不动 README。**末切片仪式依赖解锁扫描(three-tier §4 第 7 步)**:booking-state-cancel(priority 68,depends_on=device-poweron 已 passing,EP2 plan 就绪)依赖已满足 → WIP=1 下它成为合规的唯一 in_progress。验证:`./init.sh` 739 passed(基线 737+2)+ ruff 0 + vitest 27 passed + build 1.72s。/code-review 双轴:Standards 0 硬违反(修 1 判断项 hex 长度);Spec 4 AC 全满足(修 1 部分实现 booking DB 验证)。
- **booking-state-cancel 历史摘要(已 passing,详见上方 ✅ 行)**:EP2 回环(Session 143)+ 让位 priority 67→68(Session 144)+ platform-cross-tenant-write 收尾后成为 WIP=1 合规唯一 in_progress(Session 146)→ EP3 切片 01 实施并 feature 收尾(Session 147)。完整收尾记录见上方 ✅ passing 行。
- **device-models-admin-ui ✅ passing(2026-07-25 Session 142 收尾)**:2 切片全合并(切片 01 前端地基 PR #119 初版 + PR #120 Standards 修正 → **切片 02 UI 层 PR #122**)。切片 02(末切片)落地 super_admin 设备型号目录管理页:新 `components/ui/key-spec-rows.tsx`(结构化 key-value 行编辑器,支持 string/number/boolean 三类型,导出纯函数 `serializeSpecs`/`deserializeSpecs` 处理空 key 过滤/重复 key 后者覆盖/按 type 序列化/typeof 反推 type)+ `key-spec-rows.test.tsx`(vitest 7 用例覆盖 4 类 case)+ `pages/device-models-page.tsx`(参照 groups-page 骨架:PageHeader+Card+Table+3 Dialog+搜索框,react-hook-form+zodResolver;列 名称/品牌/规格摘要/¥单位成本(**内联 `toFixed(2)`,不复用 `formatCurrency`**——它 toFixed(4) 是 token cost 设计)/更新时间/操作;brand datalist 原生联想 + client-side filter 搜索)+ `App.tsx` `/device-models` Route 挂 RequireSuperAdmin + `nav-items.ts` 平台段 `Cpu` 图标 `platformOnly:true`。**实施期决策(plan §4.5 留痕)**:原写「Controller 包 KeySpecRows」,实施发现 groups-page `selectedTenantIds`(非标量数组字段)用平行 `useState` 不用 Controller,对齐既有范式改用平行 `useState<SpecRow[]>`+手动 setSpecRows 同步。`/code-review` 双轴:Standards 无硬违规(3 判断性坏味均低危);Spec 1 偏离(Controller→useState)已 plan 留痕。验证:tsc 0 错 + oxlint 0 warnings + vitest 22 passed(含新增 7)+ npm build 通过 + pytest test_device_models_api.py 22 passed 回归 + PR #122 CI 4/4 绿。**设备功能系列(61-64+66)收官**:61 device-models-crud ✅ → 62 devices-crud-ui ✅ → 63 device-booking ✅ → 64 device-poweron ✅ → 66 device-models-admin-ui ✅(61 的范围漂移修复补遗)。手测项留真实环境(对齐 devices-page/bookings-page 现状,无自动化手测)。
...
- **当前 blocker**: 无
- **EP3 断点**: **bookings-shared-split 切片链 frontier = 切片 1(未启动)**(2026-07-29 EP2 回环完成,plan + feature_list 同步,frontier 就绪)。**切片链进度**:切片 1 ⬜(frontier:抽 status-meta.ts + date-utils.ts)/ 切片 2 ⬜(抽 filter.ts + schedule-grid-card.tsx + 改消费者 deep import + shared 瘦身 + D5 回源)/ 切片 3 ⬜(末切片:收尾验证 + feature passing)。composite-chat(priority 72)全 4 切片已于 2026-07-28 Session 155 收官 passing,作为前一 frontier 归档。
