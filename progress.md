...
- **当前最高优先级未完成功能**: **chat-page-split(priority 76)✅ passing —— 3 切片全完成**(2026-07-30 Session 165,commit c2cd439 切片1 + 42e8504 切片2 + 4c961c2 切片3)。第 7 次巡检 Top recommendation,前端 chat-page.tsx(1038 行单函数)拆 ConversationListPanel + buildWorkingList 纯函数,**全部 3 切片已收官**:chat/ 文件夹 6 文件(chat-page 16 行 barrel + index 590 行路由入口 + conversation-list-panel + build-working-list + customer-helpers + __tests__ 2 文件),对称 bookings/。**frontier —— perm-backfill-dedupe 切片 02 待启(末切片)**。perm-backfill 切片 01 ✅ 本地完成(参数化合并 + caller 临时改造,commit 6461236 在 `refactor/perm-backfill-dedupe-t1` 分支,**PR #151 OPEN 待合并**,用户决策暂不合攒着和切片 02 一起),仅剩切片 02(scripts 合并 + 测试 parametrize 收尾)。另有 devices-page-split(78)规划就绪待启(**实施顺序 ①✅→③(进行中)→②**)。chat-page-split(76)✅ + union-cast-split(75)/twospace-config(74)/bookings-shared-split(73)/composite-chat(72)/principal-scope-doc-alignment(71)/principal-module(70)均 passing。
...
- **perm-backfill-dedupe 切片 01 ✅(2026-07-30 Session 166,非末切片,commit 6461236)**:service backfill 函数参数化合并 + caller 临时改造。落地 4 改文件 + 1 改 plan:① `app/services/permission_service.py`(L723+L828 两个逐字节镜像函数 `backfill_devices_perms_for_existing_tenants` + `backfill_bookings_perms_for_existing_tenants` 合并删除 → 新参数化函数 `backfill_perm_set_for_existing_tenants(db, obj)` + `BACKFILLABLE_OBJS = frozenset({"devices","bookings"})` 白名单约束 D4;循环变量改名 `perm_obj` 避 shadowing 函数参数 `obj` v2 风险点已处置;f-string 错误信息 `obj must be one of {sorted(BACKFILLABLE_OBJS)}` 防 ValueError 文案漂移)+ ② `scripts/backfill_devices_perms.py` + `scripts/backfill_bookings_perms.py`(各临时改调新函数,保留不删 —— 删除移到切片 02;v2 测试空窗消除:K chapter 保留 6 测试绿)+ ③ `tests/test_bookings_api.py` + `tests/test_devices_api.py`(各 K chapter 测试临时改调新函数,6 测试全绿保留不删 —— 删 K chapter 移到切片 02 parametrize 接管)。**验证**:`./init.sh` 冒烟全绿(scope guardrail K6 守住:白名单外 obj 抛 ValueError,不误改其他 obj perms)+ grep 旧函数**定义**归 0(引用暂留 scripts/tests,切片 02 清)。**code-review 双轴**(plan 标注):Standards 0 硬违规 / Spec AC 全满足。**PR #151 OPEN 待合并**(用户决策:非末切片暂不合并,留 `refactor/perm-backfill-dedupe-t1` 本地分支攒着和切片 02 一起合并;代码仅在该分支 + origin,未进 main)。**回归验证修正(本会话抓到)**:harness-router 回归模式此前判定本切片「✅ 真完成」时,第三源 git 检查只看了本地 commit 存在,未严格确认 PR `mergedAt` —— 实为「本地完成、PR 未合并」态。已据此改进回归流程(见下方流程改进记录)。**非末切片**(切片 02 scripts 合并 + 测试 parametrize 收尾待做),不动 feature_list.json status/evidence(末切片的事)。下一步:**切片 02 末切片**(删 2 scripts + 新建 `scripts/backfill_obj_perms.py` 接 `--obj` 必填 + `--dry-run` + 删 K chapter + 新建 `tests/test_permission_backfill.py` parametrize 3 场景 × 2 obj + 1 边界 + feature 收尾)。
- **harness 流程改进:末切片分支清理规则落地(2026-07-30 Session 166)**:用户反馈「末切片 PR 合并后本地 feature 分支残留,主分支不干净」,且回归验证时发现 perm-backfill 切片 01 PR #151 实为 OPEN 未合并(此前回归第三源只查本地 commit 未查 `gh pr mergedAt`,误判「真完成」)。改进 3 处:① `harness/docs/three-tier-workflow.md` §4 末切片收尾仪式**新增第 8 步「分支清理」**(PR merged 后 `git checkout main && git pull` + `git branch -d` + 可选删远端;铁律:先 `gh pr view --json mergedAt` 确认非 null 再删,`-D` 强删会丢代码;PR OPEN 时第 8 步暂缓记 progress.md)+ 回归验证纪律补强(第三源必须查 mergedAt,本地 commit + PR OPEN 不算已合并);② `harness/clean-state-checklist.md` 清单 9→**10 项**,新增第 10 项「无残留分支」+「没勾怎么办」表加「残留分支」行;③ progress.md perm-backfill 切片 01 条如实标注「PR #151 OPEN 待合并」态。**下次回归起,真完成判定需 PR mergedAt 非 null**(末切片收尾的第 8 步闸口)。
...
- **EP3 断点**: **perm-backfill-dedupe 切片 01 ✅ → frontier 推进到切片 02(末切片)**(2026-07-30 Session 166)。第 7 次巡检(2026-07-30 Session 164)完成 3 feature 同批 EP2 批量规划(grill + spec + tickets,plan 均 draft v2 经 opus ×3 对抗式审查修订),Phase 2 串行实施进展:① chat-page-split ✅ passing(3 切片全完成,见上条)+ ③ perm-backfill-dedupe **切片 01 ✅ commit 6461236**(service backfill 函数参数化合并 + caller 临时改造,K chapter 保留 6 测试绿),**切片 02 待启(末切片:scripts 合并 + 测试 parametrize 收尾)**。② devices-page-split 规划就绪待启。**实施顺序 ①✅→③(进行中)→②**(① Top rec 解 A2 不可测债 / ③ 最小最快 / ② 照抄 bookings 范式)。**frontier 在 ③ 的切片 02**。
  ...
  - **③ perm-backfill-dedupe(pri 77,Strong WORSENED)**:permission_service 两个 backfill 函数逐字节镜像 → 参数化合并 + 白名单防误用。**grill 4 决策**(D1 合并+改所有 caller / D2 合并 scripts[v2:改名 backfill_obj_perms.py 避碰撞] / D3 测试 parametrize / D4 Literal 白名单)。**2 切片**:① service 函数参数化 + caller 改造[v2:临时保留 6 测试绿,不删 K chapter] → ② scripts 合并 + 测试 parametrize 收尾。plan:`harness/docs/plan-perm-backfill-dedupe.md` **draft v2**(经 opus 审查修订)。风险点:scope guardrail 保持(K6)+ **循环变量 perm_obj 改名避 shadowing**(v2 新增)。
