...
- **当前最高优先级未完成功能**: **platform-cross-tenant-write(priority 67,「权限」area,总部平台角色跨店全写,status `in_progress`)**。EP2 回环已完成(Session 144),plan `harness/docs/plan-platform-cross-tenant-write.md` 落地(5 切片:01 devices 后端 frontier / 02 bookings CRUD 后端 / 03 bookings 状态机后端 / 04 前端 / 05 联调收尾,线性依赖图无环,51 条 AC)。**下一步 = EP3 实施切片 01「后端 devices 跨店写 + tenant_id 守卫 + 测试」**:从 plan §6 接 `/implement`。引入 `is_platform_writer` helper(边界={super_admin,hq_staff})+ `_resolve_target_tenant`(平台必填/门店禁带→400)+ DeviceCreate/Update 加 optional tenant_id;11 个写动作全加 platform_writer 分支。**向后兼容**:门店角色不带 tenant_id 行为零变化;**鉴权零回归**:门店角色带 tenant_id→400 防伪造。**硬约束**:范围限定 devices+bookings(不动 customers/groups/device_models)/ 不动状态机 _TRANSITIONS 表 / 不动权限基础设施(DEFAULT_*_PERMS/Role.data_scope/casbin)/ 不动 router URL 前缀。grill 6 决策 D1-D6 已全锁定,plan 自检 4 项全过。**booking-state-cancel(原 67)已让位到 68**,排在本任务之后实施。**最近 passing:device-models-admin-ui(2026-07-25 Session 142 收尾)**。
- **platform-cross-tenant-write ⏳ in_progress(2026-07-25 Session 144 EP2 回环完成)**:设备/预约功能系列(61-66)收官后的平台角色权限升格需求。**问题**:平台角色(super_admin + hq_staff)在 devices+bookings 是「跨店只读」(CROSS_TENANT_VIEWER_ROLES + permission_service.check read short-circuit);router 层 super_admin 已全 bypass,但 service body 有 owner-only/customer-only/hq_staff→403 硬分支,平台角色进了这些分支会被一并拒(无 tenant_id/不是 owner/不是 customer)。**EP2 回环**:`/grill-with-docs` 6 决策 → `/to-spec` 落 plan-platform-cross-tenant-write.md → `/to-tickets` 5 切片(§6,51 条 AC)。grill D1 body 显式 tenant_id(平台必填/门店禁带→400 BizError)/ D2 customer_id 沿用现有字段+手填/留空(customers 零改动)/ D3 三分支 elif+增强 store principal(start 可开 walk-in)+bypass require 不引临时 owner+状态机零改动 / D4 is_platform_writer 边界={super_admin,hq_staff} 与 is_cross_tenant_viewer 同(DataScope 零改动) / D5 HqView target 下拉+行内 DropdownMenu+复用 StoreView Dialog(参数化 tenantId) / D6 全矩阵+边界 22-25 用例+门店伪造拒+旧断言改写(test_hq_platform_role.py)+不动现有 A-F 章节。plan 自检 4 项全过(无环/51 AC/首片可开工/决策无悬空)。feature 登记 priority 67 area 权限 depends_on device-poweron,sync-active 刷新。**待 EP3**:接 `/implement` 实施切片 01。
...
- **当前 blocker**: 无
- **EP3 断点**: **platform-cross-tenant-write 切片 01 待实施**(plan §6,13 条 AC,Blocked by 无 = frontier)。从 plan `harness/docs/plan-platform-cross-tenant-write.md` §6 接 `/implement`,引入 is_platform_writer + _resolve_target_tenant + Device schema 加 tenant_id + device_service 写动作加分支 + 平台角色跨店写测试 + 门店伪造拒测试。**排队中**:booking-state-cancel(原 67→68 让位)切片 01 待 platform-cross-tenant-write 全 5 切片完成后再接。**巡检剩余 6 候选**(candidate 3-8)未做,candidate 2(= booking-state-cancel)已进 EP2 回环产出 plan 待实施。
