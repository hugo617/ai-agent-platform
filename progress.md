...
- **当前最高优先级未完成功能**: **risk-hardening 系列(生产加固,EP1 已完成)—— frontier = EP2 回环 `rate-limit-login-lockout`(priority 96,认证-安全,第 10 次巡检业务风险 R1 🔴:slowapi 全局 API 限流 + 登录失败锁定 + TTL 从 7 天降小时级)**。EP1 已登记 6 条 not_started(pri 96-91,总纲 [harness/docs/plan-risk-hardening-overview.md](harness/docs/plan-risk-hardening-overview.md),8 决策全用户拍板无默认采纳):96 rate-limit-login-lockout / 95 booking-toctou-guard(R2 TOCTOU DB 兜底)/ 94 chat-stream-wallet-gate(R3 前半,止血)/ 93 billing-reconciliation-job(R3 后半,挂 APScheduler 对账)/ 92 super-admin-write-audit(R4 三处最高危)/ 91 config-startup-guard(R5 三件套)。六条相互独立,WIP=1 按 priority 串行,每条 EP2 时阈值类取舍(锁定几次/多久、TTL 值、配额分级等)必须问用户拍板。系列收官 = 全 passing + 第 11 次巡检复验 Top5 清零。最近收官:**perm-check-bypass(86,权限-后端)✅ passing(2026-08-14 Session 207,全 2 切片端到端:切片 01 PR #162[commit db06891]+ 切片 02 末切片 PR #163[commit fc46275,merge a537f36],全量 1012 passed[1006 基线零回归 + 新增 6])** —— check() 内 5 层顺序敏感 bypass if 链 → `CHECK_RULES` 有序注册表,plan §5 七条不变式契约全部常驻 CI(顺序/元数据快照 + ALLOW objs 域两两互不重叠 + applies() 边界哨兵谓词直测 + ⑤ needs_db 无 db 安全降级 + verdict 短路[含 DENY 不命中反向三段证据] + casbin 终点),interface 不变全部 require/check 调用点零改动(git diff origin/main 仅白名单 3 文件),CONTEXT.md 新增术语「判定链(Decision Chain)」;不拆 permission_service 模块(第 7/8 次巡检已判 deep 关闭)。**下一步**:新会话从 EP2 接 —— `rate-limit-login-lockout`(96)单回环 `/grill-with-docs`→`/to-spec`→`/to-tickets`(阈值类取舍须用户拍板,见总纲「EP2 待拍板」);系列其余 5 条 not_started 排队;第 10 次巡检重构候选 ②-⑥ 仍排队(报告 ~/.cache/ai-agent-platform-architecture-reviews/2026-08-14.html)。**知识库分级 4-feature 系列全部 ✅ 收官**(2026-08-08):p90 foundation ✅ passing(全 3 切片)/ p89 backend ✅ passing(全 4 切片,PR #154)/ p88 reader-ui ✅ passing(全 3 切片,PR #155/#156/#157)/ **p87 admin-ui ✅ passing(全 5 切片,PR #158 切片01+02 / #159 切片03 / #160 切片04 / #161 切片05 末切片,merge commit b16222b)**。admin-ui 是系列最后一片,5 切片端到端交付:后端接缝补齐(B1 MeResponse group_admin 派生身份 + B2 DocumentCreate scope 校验 + B3 list distributions)+ 前端管理 tab 框架 + 下发 Dialog/撤回 + Category CRUD + reader category 联动。前端累计 248 测试 + 后端 1004 passed 零回归。**下一步**:EP3 从 perm-check-bypass 切片 01 接(plan §6,`/implement`,无 blocker);follow-up 候选仍排队(toast「重新激活 M 条」需后端 KnowledgeDistributionRead 加 was_reactivated 字段,独立小切片)。**网络说明**:本会话发现沙箱 `git push`(github.com:443)需 `-c http.version=HTTP/1.1 -c http.postBuffer=524288` 调整才能通(HTTP/2 被阻断,HTTP/1.1 可用);`gh`/`gh api`(api.github.com)全程可达。最近 passing:perm-check-bypass(86)/knowledge-tiered-admin-ui(87)/knowledge-tiered-reader-ui(88)/knowledge-tiered-backend(89)/knowledge-tiered-foundation(90)。
