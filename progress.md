...
- **当前最高优先级未完成功能**: **无 frontier(前端设计系统收口系列已全部收官 ✅)**。`design-system-spacing-card-hierarchy`(p83)切片 02 末切片已合并(Session 180),三片 feature 全部 passing。系列总纲 `harness/docs/plan-frontend-design-system-overview.md`「系列状态」段已更新为 ✅ 全部完成。**系列进度:① A ✅ passing / ② B ✅ passing / ③ C ✅ passing(切片 01 Session 179 + 切片 02 Session 180 末切片)**。① **A `design-system-token-foundation`(p81,✅ passing,Session 173)** — semantic token 基建。② **B `design-system-color-sweep`(p82,✅ passing,Session 178 末切片合并)** — 业务页 ~30 处硬编码色扫荡(跨 ~12 文件,全 5 切片)。③ **C `design-system-spacing-card-hierarchy`(p83,✅ passing,Session 179+180)** — 卡片层级 shadow 语义命名(**surface**/overlay [v2:原 card 因与 colors.card 碰撞改名])+ 字号任意值 text-[10px] 收口(text-2xs 扩展 + 11 处映射归零),2 切片。执行顺序 A→B→C(WIP=1,C 是系列收官)。**下一步:等用户排新需求**(无在途 frontier;feature_list.json 最高优先级 passing 任务均已完成,无 not_started)。更早 passing:design-system-color-sweep(82,B)/design-system-token-foundation(81,A)/queries-endpoints-domain-split(80)/customers-page-split(79)/devices-page-split(78)/perm-backfill-dedupe(77)/chat-page-split(76)/union-cast-split(75)/twoscope-config(74)/bookings-shared-split(73)/composite-chat(72)/principal-scope-doc-alignment(71)/principal-module(70)。
