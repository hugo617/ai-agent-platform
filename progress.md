...
> 用于在新对话中继续 devices-crud-ui 切片 04。复制下面 `---` 包裹的内容作为新对话的第一条消息。

---
/implement devices-crud-ui 切片 04:客户绑定端点(bind/unbind,幂等语义)

## 任务
实施 harness/docs/plan-devices-crud-ui.md「实施切片 → 切片 04」章节。
先读该文档的「决策摘要(bind 幂等性 / unbind 无绑定时 / 绑定端点守卫三行)」「§4 Service bind/unbind」「§6 API bind 端点」「§8 测试 F 章节」,
再读切片 04 的 acceptance criteria 逐项落地。
...
切片 03 已完成(Session 126):`DeviceHqRead` schema + Device model 3 个 relationship + `list_all_with_meta`/`get_all_with_meta`(selectinload)+ service `list`/`get` platform_role 分叉 + API GET 端点内分流 + E 章节 5 测试,**605 passed baseline**。
切到 main 的最新分支起点为切片 01+02+03 落地的代码。

## 开工流程(按 AGENTS.md,依次做,不要跳)
1. pwd 确认在仓库根目录(/Users/star/hugo/3-项目代码/project/ai-agent-platform)
2. 读 progress.md 恢复进度(尤其 Session 124 切片 01 + Session 125 切片 02 + Session 126 切片 03 记录)
...
## 本切片要做什么(逐项)
1. `app/schemas/device.py` 加 `DeviceBindRequest(customer_id: str | None)` + `DeviceBindResponse(device_id, customer_id, already_bound: bool)`
2. `app/services/device_service.py`:
   - `bind(device_id, tenant_id, customer_id, actor_id, platform_role)` → 返 `(device, already_bound: bool)`:
     - `_assert_customer_in_tenant`:走 `CustomerProfileRepository.get_for_tenant(customer_id, tenant_id)`,失败 → `BizError` 400(跨租户/不存在 customer 都走这里,合并成同一错误防枚举)
     - `device.customer_id == customer_id` 则 `already_bound=True` **不写库**(幂等);否则覆盖 `already_bound=False`
     - `require("devices","update")`(owner/admin 写,member 403)
   - `unbind(device_id, tenant_id, actor_id, platform_role)`:set `customer_id=None`,**无绑定时 no-op 不抛错**(DELETE 幂等语义),`require("devices","update")`
   - 复用切片 01 的 `_get_live_device`(tenant 隔离 + 软删过滤,跨租户 device → 404)
3. `app/api/v1/devices.py`:
   - `POST /{id}/bind` → **200**(body: `DeviceBindResponse`,**不是 201** —— device 资源已存在,bind 是赋值动作不创建新资源),`Depends(require_permission("devices","update"))`
   - `DELETE /{id}/bind` → **204**(**无绑定也 204**,DELETE 幂等,不返 404),同上守卫
4. `tests/test_devices_api.py` F 章节 8 条:
   - F1 bind 成功 → 200 + `already_bound:false`
   - F2 重复 bind 同 customer → 200 + `already_bound:true`(不写库)
   - F3 bind 不同 customer(覆盖)→ 200 + `already_bound:false`
   - F4 unbind 成功 → 204
   - F5 unbind 未绑定 device → **204**(非 404,幂等 no-op)
   - F6 bind 跨租户 customer → 400(BizError)
   - F7 bind 不存在 customer → 400(BizError)
   - F8 member bind → 403(无 devices:update)
   - 复用切片 03 的 `_seed_customer` / `_seed_device` helper(已存在)
5. 文档影响评估

## 边界(WIP=1,严格不做,做了就是越界)
- ❌ 前端任何文件(切片 05-07,`DeviceBindRequest`/`DeviceBindResponse` frontend type 留给切片 05)
- ❌ 改动切片 01 A/B/C/D/G/H + 切片 02 K + 切片 03 E 测试的行为(只新增 F 章节,不动既有)
- ❌ bind 端点用 super_admin 守卫(devices 是租户级资源,用 `require_permission("devices","update")`,**不是** group 的 `require_super_admin()`)
- ❌ 顺手重构/clean up 既存代码
- ❌ 修 customers-page HqView 对 hq_staff 不可见的既存 bug(留给后续 customer feature)

## 关键约束(项目铁律,违反即返工)
- **bind 返 200 不是 201**:device 资源已存在,bind 是赋值动作。决策表已纠正(原写 201 是错误)。body 含 `already_bound: bool` 标志区分"新绑定"vs"重复绑定"
- **unbind 无绑定 → 204 不是 404**:DELETE 幂等是 REST 惯例,避免客户端先 GET 判空再 DELETE。`unbind` service 无绑定时 no-op 不抛错
- **绑定守卫用 `require_permission("devices","update")` 不是 `require_super_admin`**:devices 是租户级资源(tenant_id FK),绑客户是本租户业务动作。与 customers 写端点范式一致。**group 的 attach/detach 用 super_admin 是因为 group 是平台级资源**(无 tenant_id),不要混淆
- **跨租户/不存在 customer 合并成同一 400**:走 `_assert_customer_in_tenant` 失败路径,都返 BizError 400,不区分"存在但跨租户"vs"不存在"(防 customer 枚举,同 device 跨租户 404 的枚举防御逻辑)
- **复用切片 03 的 `_seed_customer` helper**:已存在于 test_devices_api.py,seed 全局 Customer 行(device.customer_id 指向全局 Customer 表,不是 CustomerProfile)

## 完成定义(对照切片 04 acceptance criteria 逐项打勾)
- `DeviceBindRequest` / `DeviceBindResponse` schema 定义
- `DeviceService.bind` / `unbind` 实现(幂等语义 + `_assert_customer_in_tenant`)
- `POST /{id}/bind` → 200 + `DeviceBindResponse`;`DELETE /{id}/bind` → 204(无绑定也 204)
- F 章节 8 条测试全绿
- `./init.sh` 全绿(ruff + pytest,新增 F 章节,零回归 —— 605 baseline + 8 = 613 expected)

## 收尾(做完后必做)
1. 跑 ./scripts/sync-active-features.sh 刷新 active 视图(如果状态有变)
2. 更新 progress.md:标记切片 04 完成,下一步切片 05(前端地基)
3. 完成这个切片任务之后,给我下一个切片(05)的提示词,用于在新的对话当中
