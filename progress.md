...
- **EP3 断点**: **knowledge-tiered-backend 切片 01+02 ✅ 完成 —— frontier=切片 03 下发/撤回 API + distribute 权限码**(2026-08-07 Session 续)。切片 01(`e7297cd`)+ 切片 02(`d21f8a9`)代码 + 测试 + 收尾文档全落,`./init.sh full` 941 passed 零回归。切片 02 兑现核心:跨 scope 可见性三路径(list_visible_for + search_by_embedding)+ G1 group_admin bypass 真正接通(KnowledgeService 4 require + graph check 全加 db,此前 bypass 是「死的」)+ DocumentRead 加 tier 字段 + retrieve_knowledge 工具 include_distributed=True。**下一步:EP3 `/implement` 切片 03**(下发/撤回 API + distribute 权限码,blocked by 切片 02 已解锁 —— list_visible_for 已就位,下发的文档能被门店看到验证下发链路闭环)。
