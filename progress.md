...
plan 切片 01 标题 ✅ + 8 AC 全勾 + §4.7 实施注记(check_plan_status_sync exit 0);feature_list **不动 evidence/status 保持 in_progress**(切片 02 收尾才动);**下一步:切片 02 超管 targeted 通知 + CONTEXT.md 术语 + feature 收尾(末切片)**。PR 证据合并后补记。
