...
- **当前最高优先级未完成功能**: **queries-endpoints-domain-split(80)in_progress —— EP3 待实施**(2026-07-30 Session 170)。第 8 次巡检同批 2 候选:④ customers-page-split(79)✅ passing(本次 Session)+ ③ queries-endpoints-domain-split(80)not_started(EP2 规划完成,EP3 待启)。customers-page-split 是第 8 次巡检候选 ④ Top,store/hq 双视图范式第 4 实例(镜像 bookings/devices/chat),全 2 切片 + code-review 双轴通过。下一步:实施 ③ queries/endpoints domain 拆(expand→contract 2 切片)。
