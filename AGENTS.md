...
**触发词 `hugo`** = 入口 A 硬触发暗号,两个子模式:① 首次/无在途任务 → 探索流程(输出 3 类选项);② 本会话说「任务完成了」→ **回归验证流程**(三源交叉验真:`feature_list.json` status + `plan` checklist 勾选 + `git log` 合并 commit,验过才刷新选项,不得凭口头报告算完成)。
