#!/usr/bin/env python
"""One-shot backfill: grant ``bookings`` / ``menu:bookings`` permissions to every
existing tenant's system roles (device-booking slice 02).

What changed in slice 02 (why this script exists):
  * ``bookings`` is a brand-new object. New tenants get the perm set via
    ``seed_tenant_defaults`` automatically, but every tenant created BEFORE
    slice 02 shipped is missing ``bookings:create/read/update/delete`` (owner),
    the matching subset for admin/member, and the ``menu:bookings`` nav entry.
  * Without this backfill, the bookings feature is broken on day one for every
    existing store: even the owner gets 403 on ``GET /api/v1/bookings``.

What the script does, per tenant, idempotently:
  1. Upserts ``bookings:<act>`` permission catalogue rows (one per unit).
  2. Grants owner/admin/member the role-permission rows listed in
     ...
     (bookings-only subset).
  3. Upserts + grants ``menu:bookings`` for each system role.
  4. Re-syncs casbin from the SCD2 current state per role.

Scope guardrail: ONLY ``bookings`` and ``menu:bookings`` are touched. Re-running
it never touches other perms (``customers:read`` / ``devices:read`` etc.).
Idempotent at three layers (catalogue upsert / grant no-op / casbin rebuild).

Usage:
    python scripts/backfill_bookings_perms.py
    python scripts/backfill_bookings_perms.py --dry-run   # report only, no writes
...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if args.dry_run:
            ...
            print(f"[dry-run] Would backfill bookings perms across {len(tenants)} tenant(s):")
            for t in tenants:
                print(f"  - {t.id} ({t.name})")
            print(
